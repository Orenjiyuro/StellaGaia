...
PersonalLocalMode is for one local owner operating on their own machine and data. It removes the PB-A01-through-PB-A12 human form and all PB-I01 external compliance identity fields. R8.1 derives and validates the current repository, tool, manifest, baseline, output, and storage state directly. The user supplies only one explicit `ConfirmPersonalLocalRun` immediately before the actual long-running operation.
...
|---|---|---|---|
| PB-I01 | PersonalLocalMode derived preflight state | R8.1 in-memory/redacted state with `mode`, `derivedHead`, `derivedBranch`, `derivedToolHashMismatchCount`, `derivedManifestSourceSet`, `derivedBaselineState`, `derivedOutputBoundaryState`, `derivedRequiredFreeSpaceBytes`, `derivedAvailableFreeSpaceBytes`, safety flags, and `result`; no operator identity or approval timestamp | Recomputed from current local state; every check must pass before the single confirmation is accepted |
...
| C1-I01 | Runtime-only source-root manifest | Top level exactly `schemaVersion`, `sources`; version `1.0.0`; each source exactly `sourceId`, `sourceKind`, `rootPath` | R8.1 strictly parses locally; never copied to Git or portable output |
| C1-I02 | Machine-local immutable baseline record | Top level exactly `schemaVersion`, `inputFingerprint`, `sources`; version `1.0.0`; each source exactly `sourceId`, `sourceKind`, `rootFingerprint`; no `rootPath` | R8.1 derives `FirstCaptureNoBaseline` or validates an existing immutable baseline |
...
R8.1 derives the exact manifest source-kind/source-ID set. Duplicate IDs under `OrdinalIgnoreCase`, case drift, unavailable roots, omitted locally available approved-boundary roots, unapproved kinds, or network/runtime sources Stop.
...
| PB-SP02 manifest sources | every C1-I01 source row by exact `sourceId` | ApprovedAvailable, InvalidOrUnavailable | `manifestSourceCount = approvedAvailableCount + invalidOrUnavailableCount`; require invalid zero |
| PB-SP03 source files | every later file under ApprovedAvailable roots | Cataloged, ExplicitlyExcluded | evaluated only during LO; counts and bytes conserve |
...
| PB-FT03 | Manifest missing/malformed or sensitive projection | `S,S` | Preserve outside Git; correct locally and rerun R8.1 |
| PB-FT04 | Missing root, duplicate identity, collision, reparse point, symlink, junction, or boundary violation | `S,S` | No output; correct owning prerequisite |
...
2. strict runtime manifest shape and exact local source-kind/source-ID set;
3. baseline state (`FirstCaptureNoBaseline` or validated `ApprovedBaselinePresent`);
...
- Inspect C1-I01 locally without printing/copying its path or `rootPath` values.
...
C1-E01 records only derived/redacted state, `PersonalLocalModeConfirmationCount=1`, start/end identity, versions/hashes, source IDs/kinds, times, counts/bytes, canonical output, command identity, gate results, failure owner, and explicit downstream denials.
