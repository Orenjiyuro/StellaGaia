...
**Current baseline:** `codex/asset-corpus-integration` includes the LC-I07 and LC-I11 physical contract checkpoints plus complete fixture-only C3, C4, and C5 output paths. C0 and C1 Phase A are complete. C2 Phase A intake through SP-09 publication is implemented and its complete fixture-only path has passed completion verification. The C2 Phase B fixture-to-real runbook is written, but Phase B remains unauthorized. C3–C6 responsibilities and input/output contracts are defined; C5 derives all six risk and capability-suitability assessment states from immutable fixtures, emits the complete no-executor C5-O01/O02/O03/O04 vector, and suppresses all success artifacts on LF-15, while C3/C4/C5 publication, C6, G5 integration, Phase B execution, and C7/G4 Unity have not started.
...
C3-2, the exact-byte/Ordinal correction, and the C5 LC-I09/LC-I10 authority/freshness correction are completed checkpoints. C3 now requires every non-key support fact to be Known, derives the complete SP-31 universe, and conserves references independently from membership. C3/C4 bind their LC-I07 execution object to the exact accepted bytes; C5 binds LC-I07, LC-I11, LC-I09, and every LC-I10 package to exact accepted bytes; C3-C5 identity/output ordering is explicitly Ordinal; C5 Tuple risk identity preserves declared `sourceFactKinds` order; and evidence-generation byte changes participate in C5 LX-HI-14. The C6 prerequisite contract closure freezes C4-to-C6 actionable failure/input projection, the complete C6-O02 decision-row shape, and the LC-I12 schema/exact-byte/freshness rules. C4 now implements and conserves the typed `failureClasses`, `actionableFailureClasses`, and `availableInputKinds` projection through check, member, and family rows; dependent C5/LC-I12 fixture fingerprints are refreshed. The pure C6 child-conservation checkpoint now proves exact C3 family/member, C4 family/member, C5 risk requirement/assessment, and C5 capability-suitability requirement/assessment universes; missing, duplicate, extra, or mismatched child rows fail closed as LF-16 or LF-18 before any decision rule runs. It emits no C6 artifact and grants no C6-0 authority. C6-0 is the next separately authorized implementation Task; C6-1, C3-C5 publication, C2 Phase B execution, real assets, extraction, Unity, import, G5, and release work remain unauthorized.
