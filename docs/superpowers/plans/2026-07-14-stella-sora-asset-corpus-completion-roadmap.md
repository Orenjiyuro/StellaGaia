...
**Current baseline:** `codex/asset-corpus-integration` includes the LC-I07 and LC-I11 physical contract checkpoints, C3-0/C3-1 fixture-only family membership and output-vector execution, and C4-0/C4-1 member-static qualification and output-vector execution. C0 and C1 Phase A are complete. C2 Phase A intake through SP-09 publication is implemented and its complete fixture-only path has passed completion verification. The C2 Phase B fixture-to-real runbook is written, but Phase B remains unauthorized. C3–C6 responsibilities and input/output contracts are defined; the C3 and C4 fixture-only output paths are complete, while C3/C4 publication, C5-C6, G5 integration, Phase B execution, and C7/G4 Unity have not started.
...
LC-I13 is frozen as a strict typed lane-fact schema with a valid six-carrier fixture, schema-byte fingerprint binding, duplicate subject/kind rejection, and negative carrier coverage. The pure LC-I06 fixture-only producer is implemented against the frozen three-input sub-contract. LC-I07 and LC-I11 each have closed logical and physical contracts; LC-I11 uses an external exact-byte fingerprint and preserves the C5 strong typed capability-suitability evidence boundary. C3-0/C3-1 and C4-0/C4-1 now supply the complete fixture-only C3 family and C4 static output paths.

The next normal Task is **C5-0: add only the C5-consumed representative and capability-suitability statuses and implement the first no-Unity requirement/evidence walking skeleton** as a separate 20-30 minute checkpoint. It must emit explicit missing/unavailable states without running C7/G4 and must not infer BGM/SFX identity from LoopMode. LC-I12 and authoring reuse ledger v2 remain deferred until C6. C3/C4 publication, C2 Phase B execution, real assets, extraction, Unity, import, C6 implementation, G5, and release work remain unauthorized.
