...
**Current baseline:** `codex/asset-corpus-integration` includes the LC-I07 and LC-I11 physical contract checkpoints, complete fixture-only C3, C4, and C5 output paths, C6 child conservation, and the pure C6-0 decision walking skeleton. C0 and C1 Phase A are complete. C2 Phase A intake through SP-09 publication is implemented and its complete fixture-only path has passed completion verification. The C2 Phase B fixture-to-real runbook is written, but Phase B remains unauthorized. C6-0 applies precedence and capability separation without ledger serialization; C3/C4/C5 publication, C6 outputs, G5 integration, Phase B execution, and C7/G4 Unity have not started.
...
C3-2, the exact-byte/Ordinal correction, the C5 LC-I09/LC-I10 authority/freshness correction, C4 typed decision-input projection, and C6 child conservation are completed checkpoints. C6-0 now binds each parsed C3-O01/O02/O03, C4-O01/O02, and C5-O01/O02 execution object to the exact bytes named by its direct-input SHA, consumes exact LC-I07/LC-I08/LC-I11/LC-I12 bytes, validates the exact C6 direct-input and repair-history freshness boundary, applies deterministic family precedence, and keeps capability suitability isolated to its exact capability/route. The base fixture proves `NeedsDiagnosis/UseOriginalAsset/RepairOnce/PrototypeReplacement/RetainForLater/DiagnosticOnly/Stop = 1/0/1/0/3/0/0` and `Satisfied/Unsatisfied/Blocked = 0/1/6`; focused mutations prove prior-attempt, hard-stop, UseOriginalAsset, pool-status, and Audio capability separation behavior. C6-0 emits no lifecycle artifact. C6-1 authoring-ledger output remains the next separately authorized Task; C3-C5 publication, C2 Phase B execution, real assets, extraction, Unity, import, G5, and release work remain unauthorized.
