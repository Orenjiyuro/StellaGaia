...
**Current baseline:** `codex/asset-corpus-integration` includes the LC-I07 and LC-I11 physical contract checkpoints, complete C3/C4 fixture-only output paths, and the C5-0 no-Unity requirement/evidence walking skeleton. C0 and C1 Phase A are complete. C2 Phase A intake through SP-09 publication is implemented and its complete fixture-only path has passed completion verification. The C2 Phase B fixture-to-real runbook is written, but Phase B remains unauthorized. C3–C6 responsibilities and input/output contracts are defined; C5 now derives explicit Required/Missing/Unavailable risk and capability-suitability states, while C3/C4 publication, the complete C5 output/failure vector, C6, G5 integration, Phase B execution, and C7/G4 Unity have not started.
...
LC-I13 is frozen as a strict typed lane-fact schema with a valid six-carrier fixture, schema-byte fingerprint binding, duplicate subject/kind rejection, and negative carrier coverage. LC-I07 and LC-I11 each have closed logical and physical contracts. C3-0/C3-1 and C4-0/C4-1 supply the complete fixture-only C3 family and C4 static output paths, and C5-0 supplies the first runnable no-Unity requirement/evidence path with strong capability-suitability separation.

The next normal Task is **C5-1: complete immutable fixture evidence assessment for Accepted/Stale/Rejected states without executing C7/G4** as a separate 20-30 minute checkpoint. It must preserve capability/route isolation and exact input freshness; the complete C5 output vector remains a later checkpoint unless it fits that independently reviewed slice. LC-I12 and authoring reuse ledger v2 remain deferred until C6. C3/C4 publication, C2 Phase B execution, real assets, extraction, Unity, import, C6 implementation, G5, and release work remain unauthorized.
