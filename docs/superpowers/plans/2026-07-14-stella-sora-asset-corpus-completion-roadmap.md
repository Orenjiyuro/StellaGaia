...
**Current baseline:** `codex/asset-corpus-integration` includes the LC-I07 and LC-I11 physical contract checkpoints, fixture-only C3-C6 output paths through C6-O04, root summary schema 2.0.0, and the lightweight fixture-only G5 freshness-validating aggregator. C0 and C1 Phase A are complete. C2 Phase A intake through SP-09 publication is implemented and its complete fixture-only path has passed completion verification. The post-R6 C3-C5 exact-byte authority corrections and same-generation fixture-only C2→C6→G5 harness are complete, and a fresh R6 completion audit passes at code baseline `e5effee`. The C2 Phase B fixture-to-real runbook is written, and the separate R7 package now freezes the only currently eligible Phase B request as one diagnostic real C1 snapshot with twelve exact human confirmations. Phase B remains unauthorized; the next state is waiting for that exact human authorization. The authoring reuse ledger is upgraded to v2; lifecycle publication, Phase B execution, and C7/G4 Unity have not started.
...
Preparation record: `docs/asset-migration/source-corpus-phase-b-authorization-package.md` freezes the target user and path, central Artifact/Subject/Failure registries, exact C1 command/output boundary, current tool and contract hashes, source-kind vocabulary, runtime-only manifest boundary, disk-budget formula, rollback/quarantine behavior, sensitive-path policy, acceptance evidence, and PB-A01 through PB-A12 confirmation rows. It grants no execution authority. Real C2 remains blocked by its absent reviewed adapter, producer set, and real publication-root contract. The next state is **waiting for Phase B exact human authorization**; broad permission does not start R8.
