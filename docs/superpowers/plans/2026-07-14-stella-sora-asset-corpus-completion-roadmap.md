...
C3-2 is the current completed correction checkpoint: C3 now requires every non-key support fact to be Known, uses the accepted AR-O04 selector set only for the authoritative known-empty dependency case, derives Dependency references from LC-I01, emits all five SP-31 reference kinds, and proves Resolved/Missing/Conflict conservation independently from membership. The next normal Task is a separate **exact-byte execution-object binding and Ordinal ordering correction** checkpoint. C6-0 and C6-1 are not approved; C5 evidence authority, C6 child conservation, LC-I12 closure, specification conflicts, C3-C5 publication, C2 Phase B execution, real assets, extraction, Unity, import, G5, and release work remain unauthorized.
