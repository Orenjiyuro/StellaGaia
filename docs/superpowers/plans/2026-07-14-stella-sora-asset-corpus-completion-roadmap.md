...
**Current baseline:** `codex/asset-corpus-integration` includes the LC-I07 and LC-I11 physical contract checkpoints, complete independently audited fixture-only C3-C6 output paths through C6-O04, root summary schema 2.0.0, and the lightweight fixture-only G5 freshness-validating aggregator. C0 and C1 Phase A are complete. C2 Phase A intake through SP-09 publication is implemented and its complete fixture-only path has passed completion verification. The independent R6 C0-C6 fixture-only Phase A completion audit is complete. The C2 Phase B fixture-to-real runbook is written, but Phase B remains unauthorized. The authoring reuse ledger is upgraded to v2; lifecycle publication, Phase B execution, and C7/G4 Unity have not started.
...
C3-2, the exact-byte/Ordinal correction, the C5 LC-I09/LC-I10 authority/freshness correction, C4 typed decision-input projection, C6 child conservation, C6-0 decision evaluation, C6-1 output generation, the independent C3-C6 fixture-only completion audit, the R5 root-summary 2.0.0 contract change, the R5 lightweight G5 aggregator, and the independent R6 C0-C6 fixture-only Phase A completion audit are completed checkpoints. The audited base generation proves family partition `1/0/1/0/3/0/0`, member pools `0/0/5`, capabilities `0/1/6`, Passed `5/5/0`, LF-16/LF-17/LF-18 diagnostic-only `5/1/4`, and independent coverage/readiness projection. G5 additionally proves exact-byte freshness across its four direct artifacts, C6-only C3-C6 consumption, shared-snapshot conservation, schema-valid in-memory root output, and zero executor/heavy/publication activity. The next normal Task is R7: prepare the explicit Phase B authorization package without executing Phase B. Lifecycle publication, C2 Phase B execution, local source access, real assets, extraction, Unity, import, C7/G4, and release work remain unauthorized.
