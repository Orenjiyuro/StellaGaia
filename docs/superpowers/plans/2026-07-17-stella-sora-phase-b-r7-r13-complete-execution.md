...
- `CERG-T0` completed at `ordinaryTaskUsed=1/6`; CERG-T1A consumed the second slot and CERG-T1B consumed the third, so current counters are `ordinaryTaskUsed=3/6`, `LOUsed=0/3`. The historical T1-v1 evaluation and its purported T4 conclusion are both `InvalidatedByContractDefect` and consumed no valid route slot. TO01/TO02/TO03/TO04 corrections, the non-artifact T1B digest/binding diagnostics, and the current finite-attachment contract correction consume no additional slot; an audited corrected T1B re-execution remains inside the already consumed third slot.
...
5. `CERG-T1B` consumed the third ordinary slot. Its digest false negative and overbroad all-Transform-to-base-skeleton rejection are non-artifact diagnostics, not candidate or terminal results. The current docs-only correction freezes the finite attachment partition and stops. Only after total control audits this corrected contract may a separately authorized same-slot T1B re-execution at `ordinaryTaskUsed=3/6`, `LOUsed=0/3` revalidate the allowed C1/FFS evidence, lock only `char_14401`, freeze the eleven obligations including the two exact attachment rows, and atomically publish T01/O01; it cannot modify TG01-TG03 or TO01-TO04 and must stop before preflight.
