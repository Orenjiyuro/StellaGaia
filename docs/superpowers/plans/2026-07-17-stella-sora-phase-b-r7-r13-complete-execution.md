...
4. `CERG-T1A` completed at `ordinaryTaskUsed=2/6`, `LOUsed=0/3`; its original TO01 is immutable `RejectedByTotalControlAudit` history. The in-slot audit correction updates only TG01-TG03, runs the fifteen TO02 fixture classes, commits their exact bytes, publishes TO02 without reading C1/FFS candidate evidence or creating T01/O01, and stops for independent TO02 audit.
5. Only after total control approves the exact TO02 SHA and corrective HEAD may separately authorized `CERG-T1B` read the allowed C1/FFS evidence, project the exact candidate members, lock `char_14401`, freeze the eleven discovery obligations, and publish T01/O01. It cannot modify TG01-TG03, TO01, or TO02, completes at `ordinaryTaskUsed=3/6`, `LOUsed=0/3`, and stops before preflight.
...
Apart from the registered TO02 correction artifacts above, there are no other v2.2, tooling, preflight, LO-CERG1, repair, revalidation, or early-terminal artifacts or implementation roles. The original TO01 is rejected immutable history. Only after total control approves the exact Green TO02 SHA and corrective HEAD may a separately authorized `CERG-T1B` read the allowlisted C1/FFS evidence and create T01/O01; it may not modify TG01-TG03, TO01, or TO02 or accept a machine-local substitute. A Failed or unaudited TO02 suppresses T1B and O01. A01 is created before any source read or staging/Work/Output member; therefore `A01 absent` means the LO attempt never began, while `A01 present and valid R01 absent` has exactly one meaning and transition.
...
- O01 freshness binds the corrective tool HEAD, equal I01/I02 raw hashes, all I01-I04 hashes/member-set fingerprints, raw TO02 SHA, every corrected TG01-TG03 raw SHA from TO02, the two candidate fingerprints, and the baseline fingerprint. Any byte, membership, sort, test result, or HEAD change suppresses consumption; nothing is refreshed in place.
...
The T1A audit correction authorizes no T1B, candidate evidence read, T01/O01, preflight, source read, AssetRipper, Unity, or LO. After corrected TG01-TG03 and immutable TO02 are committed and pushed, the sole next action is `AwaitTotalControlAuditBeforeCERGT1B`.
