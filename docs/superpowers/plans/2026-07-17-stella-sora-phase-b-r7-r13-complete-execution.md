...
`inputArtifactRefs` rows are `{artifactId,portableRelativePath,byteCount,sha256}` sorted by artifactId. `selectorRows` are exactly `{selectorId,sourceId,portableRelativePath,ledgerByteCount,ledgerSha256}` in the fixed order below. `locationRows` are `{locationId,selectorId,currentByteCount,currentSha256,serializedTypeName,serializedObjectId,objectName,proofKind,proofLocator,status}` sorted by `locationId`; `serializedTypeName` must be `AnimatorController`, and `proofKind` must be `ParsedSerializedObject`, never filename/string inference. `locationId="CTRLLOC-" + sha256(CJ(["mbacr/controller-location/1",selectorId,currentByteCount,currentSha256,serializedTypeName,serializedObjectId,objectName,proofLocator]))`. `partitions={selectorCount,locatedCount,notLocatedCount,invalidCurrentLeafCount}` and `limits={maxFilesRead,maxBytesRead,maxDurationSeconds,maxResultRows,maxChildProcesses}` contain JSON integers. `producerBinding` and `validatorBinding` each have `{artifactId,role,portableTrackedPath,implementationVersion,byteCount,sha256,runtimeProduct,runtimeVersion,runtimeByteCount,runtimeSha256}`; their tracked paths, code hashes, and implementations must differ. `failures` rows are `{failureId,selectorId,reasonCode,detail}` sorted by `failureId`, with reasonCode in `[InputIdentityFailure,ParserFailure,LimitExceeded,ConservationFailure,ProducerValidatorDisagreement,OutputIntegrityFailure]`, `detail` containing no absolute path, and `failureId="MBACRF-"+sha256(CJ(["mbacr/failure/1",selectorId,reasonCode,detail]))`. L01 status is `ControllerLocated` only when `locatedCount∈{1,2}`, failures is empty, and nextAction is `AwaitTotalControlAuditBeforeMBACRT2`; otherwise status is `Failed`, failures is nonempty, and nextAction is `RecoveryRouteFailed`.
...
| `MBACR-FT03` | Locator exceeds a limit, reads outside S01/S02, uses filename/string-only proof, producer/validator disagree, any selector is `InvalidCurrentLeaf`, `locatedCount=0`, conservation fails, or L01 publication fails | Failed/absent L01 as physically observed; `3/5,0/1`; no P01 | `RecoveryRouteFailed` |
