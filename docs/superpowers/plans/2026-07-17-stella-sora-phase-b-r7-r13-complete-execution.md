...
- `CERG-T0` completed at `ordinaryTaskUsed=1/6`; the sole valid CERG-T1A slot is now consumed, so current counters are `ordinaryTaskUsed=2/6`, `LOUsed=0/3`. The historical T1-v1 evaluation and its purported T4 conclusion are both `InvalidatedByContractDefect` and consumed no valid route slot. TO01/TO02 corrections and the current docs-only TO03 registration consume no additional slot.
...
3. TO01 and TO02 are independently rejected tooling-readiness history. This docs-only correction registers the separately authorized in-slot CERG-T1A3 correction and TO03 generation without changing the counters.
4. `CERG-T1A` remains consumed exactly once at `ordinaryTaskUsed=2/6`, `LOUsed=0/3`. TO01 and TO02 are immutable `RejectedByTotalControlAudit` history. The current docs-only correction registers TO03 and stops; after its audit, separately authorized `CERG-T1A3` may change only TG02/TG03, run the twenty-two TO03 fixture classes, commit their exact bytes, publish TO03 without reading C1/FFS candidate evidence or creating T01/O01, and stop for independent TO03 audit.
5. Only after total control approves the exact TO03 SHA and its corrective tool HEAD may separately authorized `CERG-T1B` read the allowed C1/FFS evidence, project the exact candidate members, lock `char_14401`, freeze the eleven discovery obligations, and publish T01/O01. It cannot modify TG01-TG03 or TO01-TO03, completes at `ordinaryTaskUsed=3/6`, `LOUsed=0/3`, and stops before preflight.
...
`CERG-T1A` and its in-slot `CERG-T1A3` correction may use only tracked plans/contracts/tool metadata and synthetic fixtures; neither can read C1/FFS candidate evidence. `CERG-T1B` keeps every source and historical-evidence input read-only; its only machine-local writes are non-overwriting T1V22-T01/T1V22-O01. T1B's complete historical evidence allowlist is:
...
This v2.2 section is the sole executable CERG-T1A/T1A3/T1B contract. It supersedes every later v2.1, v2.0, and v1 section in full; those sections and artifacts remain immutable history. The current counters are `ordinaryTaskUsed=2/6`, `LOUsed=0/3`; the newest dated subsection controls readiness artifacts and next action.
...
- TO03 is Green-only: `status=Green`, `failures=[]`, `partitions={passedCount:22,failedCount:0,totalCount:22}`, and `nextAction=AwaitTotalControlAuditBeforeCERGT1B`. A failed, incomplete, skipped, reordered, or prerequisite-suppressed matrix produces no TOT03/TO03 and no tool commit; there is no Failed TO03 form.
...
- For each output member, `obligationRefIds` equals the duplicate-free Ordinal union of originObligationIds from only the evidence, scope, subject, and relationship rows whose authoritative locator is that member or its exact paired `.meta`; `subjectRefIds` follows the same ownership rule. Blanket attribution to all eleven obligations is forbidden. Every non-support output member must own at least one typed row; an unattributed, multiply owned, or mismatched locator is `OutputIntegrityFailure`.
...
Fixed counterexamples execute from zero and assert exact row values, not only Closed/counts: two or more events with different time/function/FX references and reordered elements; OneD thresholds distinct from ordinals; 2D positions; Direct parameter identities/values; nondefault timeScale/cycleOffset/mirror; nested and cyclic BlendTrees; AttackAction→BlendTree→nested BlendTree→leaf clip→event→FX; real `.mat` GUID references to `.png + .png.meta` and `.shader + .shader.meta`; one model prefab plus one attack-FX prefab in the same output; missing/duplicate external GUID ownership; and per-output obligation/source attribution. Any semantic mismatch suppresses TO03. `ApprovedForCERGT1B=false` until total control separately approves the exact TO03 SHA and corrective tool HEAD.
