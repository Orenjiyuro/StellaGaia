...
`CERG-T1` keeps every repository and historical-evidence input read-only. Its only permitted writes are the exact non-overwriting T1-T01/T1-O01 publication defined below. Its complete historical evidence allowlist is:
...
### CERG-T1 central contract

The definitions in this section are the only T1 Artifact Registry, Subject/Partition Registry, Failure Transition Table, identity model, and output contract. No child plan or implementation may add fields, states, artifacts, or transitions without first revising this normative override and its mirror.
...
| `T1-T01` | `Extracted/CERG/SingleCharacter/T1/candidate-lock.json.tmp` | Temporary bytes with exactly the T1-O01 shape | Must be absent at start; created only after in-memory validation and sensitive-data scan; never consumable; removed by same-Task rollback or atomically renamed to T1-O01 |
| `T1-O01` | `Extracted/CERG/SingleCharacter/T1/candidate-lock.json` | Sole frozen T1 result, schema `cerg-t1-candidate-lock/1.0.0` | Must be absent at start and is never overwritten. `Passed` is consumable only under the LO rule below; `Failed` is diagnostic-only; contract/input/safety failures suppress it entirely |

There are no other T1 inputs, intermediate files, outputs, summaries, locks, journals, or diagnostics. T1 may create the parent directories for T1-T01/T1-O01 only. A pre-existing T1-T01 or T1-O01 is a stop condition and grants no overwrite, cleanup, or rerun authority.

#### Complete T1-O01 shape

T1-O01 is one JSON object with exactly these top-level fields in this order; every field is mandatory: `schemaVersion`, `artifactId`, `contractHeadCommit`, `normativeOverrideSha256`, `mirrorOverrideSha256`, `evidenceSetFingerprint`, `status`, `consumableForLoCerg1`, `selectedCandidateId`, `inputArtifacts`, `evidenceItems`, `sourceRoots`, `candidates`, `subjects`, `relationships`, `discoveryObligations`, `failures`, `summary`, `nextAction`.

- `schemaVersion` is exactly `cerg-t1-candidate-lock/1.0.0`; `artifactId` is exactly `CERG-T1-O01`; `contractHeadCommit` is exactly 40 lowercase hexadecimal characters.
...
- `evidenceItems` rows have exactly: `evidenceId`, `inputArtifactId`, `evidenceClass`, `portableRelativePath`, `byteCount`, `sha256`, `locatorKind`, `locator`, `referenced`. `evidenceClass` is `C1SafeProjection`, `FileTreeIdentity`, `UnityMeta`, `SerializedRelationship`, `Manifest`, or `FFSRunRecord`; `locatorKind` is `C1SourceProjection`, `YamlObjectPath`, `UnityGuidRecord`, `ManifestEntry`, `Utf8LineRange`, or `WholeFileIdentity`; `referenced` is boolean. For `WholeFileIdentity`, bytes/hash identify the complete file; otherwise they identify the canonical bytes of exactly the selected safe projection. The path is repository-relative with `/` separators. The locator is a non-empty portable string and never contains an absolute path, user/host name, credential, or unrestricted log text.
...
- `subjects` rows have exactly: `subjectId`, `candidateId`, `subjectKind`, `unityGuid`, `serializedFileId`, `sourceObjectId`, `portableRelativePath`, `contentSha256`, `evidenceRefIds`. `subjectKind` is exactly one of `Model`, `SkinnedMesh`, `Renderer`, `Mesh`, `Material`, `Texture`, `Shader`, `Skeleton`, `Bone`, `Avatar`, `ActionClip`, `Controller`, `OverrideController`, `ActionState`, `AttackAction`, `FXPrefab`, `FXMaterial`, `FXTexture`, `FXShader`, or `FXAnimation`. Identity fields not applicable to the subject are JSON `null`; at least one of `unityGuid`, `serializedFileId`, `sourceObjectId`, `portableRelativePath`, or `contentSha256` is non-null. `evidenceRefIds` is non-empty. Every controller state is represented exactly once as `ActionState` or `AttackAction`, never both; that partition is the identity-bearing complete action-state list.
...
- `relationships` rows have exactly: `relationshipId`, `candidateId`, `slotOrdinal`, `sourceSubjectId`, `relationshipKind`, `targetSubjectId`, `state`, `authorityKind`, `authorityScopeId`, `authorityScopeFingerprint`, `evidenceRefIds`, `conflictingTargetSubjectIds`, `obligationId`. `relationshipKind` is exactly one of `ModelUsesRenderer`, `RendererUsesMesh`, `RendererUsesMaterial`, `MaterialUsesTexture`, `MaterialUsesShader`, `ModelUsesSkeleton`, `SkeletonContainsBone`, `AvatarUsesSkeleton`, `ControllerDefinesState`, `OverrideMapsClip`, `ActionUsesClip`, `AttackTriggersFX`, `PrefabContainsObject`, `FXUsesMaterial`, `FXUsesTexture`, `FXUsesShader`, or `FXUsesAnimation`. `authorityKind` is `Controller`, `OverrideController`, `Prefab`, `Event`, or `SerializedObject`; C1 and FFS rows may support provenance but can never be a relationship authority or prove exhaustive absence.
- `state` is `ProvenPresent`, `ProvenAbsent`, `EvidenceUnavailableBeforeExtraction`, or `Contradictory`; `slotOrdinal` is a positive integer unique for `(candidateId,sourceSubjectId,relationshipKind)`. `authorityScopeId` is a non-empty portable NFC string or null; `authorityScopeFingerprint` is a SHA-256 or null; they are null or non-null together. Every source, target, conflicting target, authority-container, and owner subject reference resolves inside the row's candidate; cross-candidate references are invalid.
- Authority pairs are closed: `ModelUsesRenderer`, `RendererUsesMesh`, `RendererUsesMaterial`, `MaterialUsesTexture`, `MaterialUsesShader`, `ModelUsesSkeleton`, `SkeletonContainsBone`, `AvatarUsesSkeleton`, and `ActionUsesClip` use `SerializedObject`; `ControllerDefinesState` uses `Controller` or `SerializedObject`; `OverrideMapsClip` uses `OverrideController` or `SerializedObject`; `AttackTriggersFX` uses `Controller`, `Event`, `Prefab`, or `SerializedObject`; `PrefabContainsObject` uses `Prefab` or `SerializedObject`; `FXUsesMaterial`, `FXUsesTexture`, `FXUsesShader`, and `FXUsesAnimation` use `Prefab` or `SerializedObject`. Any other pair is invalid. `ProvenAbsent` additionally forbids a non-container authority and requires a scope fingerprint over the complete enumerated owner collection.
- A `ProvenPresent` relationship has one non-null target, non-empty evidence, empty conflicts, null obligation, and an optional scope pair. A `ProvenAbsent` relationship has null target, non-empty evidence, empty conflicts, null obligation, and non-null exhaustive `authorityScopeId` plus `authorityScopeFingerprint`. An `EvidenceUnavailableBeforeExtraction` relationship has null target, non-empty evidence identifying the owner/container, empty conflicts, exactly one non-null obligation, and a null scope pair. A `Contradictory` relationship has null target, at least two distinct sorted conflicting targets, at least two evidence refs, null obligation, and a null scope pair.
- `discoveryObligations` rows have exactly: `obligationId`, `candidateId`, `relationshipId`, `ownerSubjectId`, `queryKind`, `authorityContainerSubjectId`, `serializedType`, `propertyPath`, `allowedReadRoots`, `expectedSubjectKinds`, `expectedRelationshipKinds`, `maxResultRows`, `terminalStates`. `queryKind` is `ResolveGuid`, `EnumerateControllerStates`, `EnumerateOverrideMap`, `EnumeratePrefabReferences`, `EnumerateAnimationEvents`, `EnumerateSerializedObjectReferences`, `ResolveMaterialDependencies`, or `ResolveFXDependencies`. `allowedReadRoots` is a sorted non-empty set of exact `sourceRoots.sourceId` values—not paths; `expectedSubjectKinds` and `expectedRelationshipKinds` are sorted non-empty sets from their respective enums and freeze every kind the query may materialize; `maxResultRows` is a positive integer; `terminalStates` is exactly `["ProvenPresent","ProvenAbsent","Contradictory"]`. Nullable selector fields are present as JSON `null`, never omitted.
- `authorityContainerSubjectId` is always non-null. `serializedType` and `propertyPath` are both non-null exactly for `EnumerateSerializedObjectReferences` and both null otherwise. The authority-container kind is `Controller` for `EnumerateControllerStates`, `OverrideController` for `EnumerateOverrideMap`, `FXPrefab` for `EnumeratePrefabReferences`, `ActionClip` or `FXAnimation` for `EnumerateAnimationEvents`, `Material` or `FXMaterial` for `ResolveMaterialDependencies`, and `AttackAction` or `FXPrefab` for `ResolveFXDependencies`; `ResolveGuid` and `EnumerateSerializedObjectReferences` may use any candidate-local subject kind. `EnumerateControllerStates` must include `ActionState` and `AttackAction` in `expectedSubjectKinds` and `ControllerDefinesState`, `ActionUsesClip`, and `AttackTriggersFX` in `expectedRelationshipKinds`, so an unseen action/attack universe cannot pass vacuously.
- `failures` rows have exactly: `failureId`, `transitionId`, `candidateId`, `subjectId`, `relationshipId`, `evidenceRefIds`. `candidateId` is non-null exactly for T1-FT07/08/09 and JSON `null` exactly for T1-FT10; nullable subject/relationship references are always present as JSON `null`. `transitionId` is T1-FT07, T1-FT08, T1-FT09, or T1-FT10 only. T1-FT07 has a non-null relationship resolving to the serialized `Contradictory` row. T1-FT08 references its unavailable relationship when that row is schema-valid; T1-FT09 references the nearest valid subject when one exists. Candidate-owned failure evidence is non-empty; gate-owned T1-FT10 evidence may be empty. T1-FT01 through T1-FT06 and T1-FT11/12 suppress output and therefore never appear; T1-FT13 is success and never appears as a failure row.
- `summary` has exactly: `sourceRootCount`, `candidateCount`, `lockedCount`, `rejectedCount`, `subjectCount`, `relationshipCount`, `provenPresentCount`, `provenAbsentCount`, `evidenceUnavailableCount`, `contradictoryCount`, `obligationCount`, `evidenceItemCount`, `referencedEvidenceItemCount`, `unreferencedEvidenceItemCount`, `failureCount`. Every value is a non-negative integer derived from the arrays, never hand-entered.
...
- `subjectId = "t1s:" + sha256(CJ(["cerg-t1/subject-id/1",candidateId,subjectKind,unityGuid,serializedFileId,sourceObjectId,portableRelativePath,contentSha256]))`.
...
- `relationshipId = "t1r:" + sha256(CJ(["cerg-t1/relationship-id/1",candidateId,slotOrdinal,sourceSubjectId,relationshipKind,authorityScopeId]))`.
- `obligationId = "t1o:" + sha256(CJ(["cerg-t1/obligation-id/1",relationshipId,queryKind,authorityContainerSubjectId,serializedType,propertyPath,allowedReadRoots,expectedSubjectKinds,expectedRelationshipKinds,maxResultRows]))`.
- `failureId = "t1f:" + sha256(CJ(["cerg-t1/failure-id/1",transitionId,candidateId,subjectId,relationshipId,evidenceRefIds]))`.
- `evidenceSetFingerprint = sha256(CJ(["cerg-t1/evidence-set/1", evidenceItems projected in their frozen sort order to [evidenceId,inputArtifactId,evidenceClass,portableRelativePath,byteCount,sha256,locatorKind,locator]]))`; the output-derived `referenced` bit is intentionally excluded. Raw registered byte-range and whole-file SHA-256 values hash exactly those bytes without a domain prefix and are never reused as a structured digest. The T1-O01 content fingerprint is the lowercase SHA-256 of its complete canonical bytes and is reported externally with the exact human-confirmation request; it is not embedded recursively in T1-O01.
- Sort `inputArtifacts` by `artifactId`; `evidenceItems` by `(portableRelativePath,locatorKind,locator,evidenceId)`; `sourceRoots` by `sourceId`; `candidates` by `(comparisonOrdinal,candidateId)`; `subjects` by `(candidateId,subjectKind,subjectId)`; `relationships` by `(candidateId,sourceSubjectId,relationshipKind,slotOrdinal,relationshipId)`; obligations by `(candidateId,relationshipId,obligationId)`; failures by `(transitionId,candidateId,subjectId,relationshipId,failureId)`. All referenced ID arrays use Ordinal ascending order.
...
- Source-root universe `U` is the non-empty set of all `sourceRoots` rows and equals exactly the identity-unique T1-I03 source projection. Each source row has exactly one ledger evidence row, every `allowedReadRoots` member belongs to U, and unused U rows remain present. Zero or duplicate source IDs, or a source-kind/root-fingerprint/count/byte disagreement, suppresses output.
- Subject universe `S` is all `subjects` rows and partitions by the twenty `subjectKind` values above. The action-state sub-universe partitions exactly as `ActionStates = ActionState ⊎ AttackAction`. Every subject belongs to exactly one candidate and one kind; cross-candidate subject reuse is forbidden even when byte identities match.
...
- Required relationship slots are exactly the union of every authority-explicit serialized reference slot and these semantic anchors: Model/SkinnedMesh to renderer and skeleton; each Renderer to its mesh and every material slot; each Material/FXMaterial to its shader and every texture property; Skeleton to every declared bone; Avatar to skeleton; each Controller to every action state; OverrideController to every override entry; every `ActionState`/`AttackAction` to its clip; every `AttackAction` to its FX expectation; and each FX prefab/object to every referenced FX material, texture, shader, and animation. A not-yet-enumerable anchor is one `EvidenceUnavailableBeforeExtraction` row with one bounded obligation whose `expectedSubjectKinds` and `expectedRelationshipKinds` freeze the only rows it may materialize. LO-CERG1 may resolve only these rows/obligations and may not discover an unregistered candidate, root, query kind, subject kind, or relationship kind.
- The T1 frozen universe is therefore the exact closure-request universe—not a claim that pre-extraction bytes already reveal every final target. `ProvenAbsent` means an identified authority scope was exhaustively read and proves zero matching reference; “the allowlisted evidence does not expose it” is always `EvidenceUnavailableBeforeExtraction` with an exact obligation. If neither exhaustive absence nor an exact owner/container/query/root/cap obligation can be represented, the candidate is Rejected under T1-FT08/09.
- Obligation universe `O` is in a one-to-one, onto mapping with `EvidenceUnavailableBeforeExtraction`: every such relationship names exactly one obligation, every obligation is named by exactly one such relationship, and therefore `|O|=|EvidenceUnavailableBeforeExtraction|`.
- Evidence universe `E` is all `evidenceItems` and partitions exactly as `E = Referenced ⊎ Unreferenced`; `referenced` is true if and only if the evidence ID occurs in any source-root, subject, relationship, or failure evidence field. Unreferenced allowlisted evidence is retained and counted, never silently discarded. Every evidence reference resolves to exactly one E row.
...
- A candidate may be Locked only when it has at least one Model or SkinnedMesh, Skeleton, Avatar, ActionClip, and Controller subject; contains no Contradictory relationship; every unavailable relationship has its exact obligation; every subject and relationship evidence reference resolves; and all candidate-local conservation rules hold. In addition, every Controller has either an exhaustively fingerprinted complete `ControllerDefinesState` scope or exactly one bounded `EnumerateControllerStates` obligation. Every known AttackAction has exactly one `AttackTriggersFX` slot. If no AttackAction is yet known, that controller-state obligation must include `AttackAction` in `expectedSubjectKinds` and `AttackTriggersFX` in `expectedRelationshipKinds`; therefore actions and attacks cannot disappear through an empty pre-extraction set, while an authoritative resolved `ProvenAbsent` relationship may still establish `NoEffectExpected` without inventing an FX subject. CERG-T2 final closure requires at least one action state and at least one AttackAction after all obligations resolve.
- Candidate-owned pre-freeze rejection may serialize its candidate row and failure row without serializing the malformed/unclassifiable subject, relationship, or obligation that caused rejection; every row that is serialized must remain schema-valid and conserved.
...
| `T1-FT05` | Unknown/missing field or enum, invalid ID/hash/path, incompatible evidence-class/locator or relationship/authority pair, unresolved evidence reference, duplicate or misordered row; gate-owned | none | removed / suppressed | denied | `CorrectCERGT1ContractOrImplementation` |
| `T1-FT06` | Any candidate/subject/relationship/obligation/evidence/failure conservation formula fails; gate-owned | none | removed / suppressed | denied | `CorrectCERGT1ContractOrImplementation` |
...
| `T1-FT08` | Unavailable relationship has zero/multiple/open-ended obligation, an unknown expected subject/relationship kind, an `allowedReadRoots` source ID outside U, or a non-positive cap; candidate-owned | candidate becomes Rejected; one failure row | may continue / final artifact determined after candidate evaluation | denied for that candidate | `EvaluateNextCandidateOrFinalizeFailed` |
| `T1-FT09` | Candidate lacks the required Model/SkinnedMesh, Skeleton, Avatar, ActionClip, or Controller minimum; lacks an action-universe/attack-universe anchor; or has an unclassified required relationship; candidate-owned | candidate becomes Rejected; one failure row | may continue / final artifact determined after candidate evaluation | denied for that candidate | `EvaluateNextCandidateOrFinalizeFailed` |
...
| `T1-FT11` | Multiple Locked candidates, more than three evaluated candidates, duplicate candidate identity, or selector broadening after lock; gate-owned | invalid | removed / suppressed | denied | `CorrectCERGT1ContractOrImplementation` |
...
- `LO-CERG1Authorized` is never stored as true. Exact human confirmation may authorize LO-CERG1 only when T1-O01 is Passed, `consumableForLoCerg1=true`, one candidate is Locked, the output content hash and every freshness binding are revalidated, T1-T01 is absent, and the confirmation names the selected candidate ID, T1-O01 hash, obligation IDs, each allowed source ID plus its privately resolved exact read-only filesystem root, operation/tool identity, attempt cap, output/storage cap, cancellation, rollback, and no-retry rule. The private root mapping is never serialized into T1-O01 or repository content.
...
Before T1 may be approved, independent review must execute this fixed matrix against the actual model. These are data vectors, not keyword-presence checks: start from one complete canonical minimal Passed object; for each negative case apply only the stated mutation, recompute every unaffected derived ID/digest/summary, and assert the exact transition and output vector. A case that reaches an earlier unstated transition is an invalid fixture, not a pass. The expected transition is exclusive; `suppressed` means zero T1-O01 and zero LO-consumable output.

| Case | Counterexample or positive vector | Expected transition | T1-O01 / LO result |
|---|---|---|---|
| `T1-FC01` | missing/invalid UTF-8 override, HEAD mismatch, or one-character mirror drift | T1-FT01 | suppressed / denied |
| `T1-FC02` | missing input, reparse point, outside-root member, or attempted parse of an unallowed class | T1-FT02 | suppressed / denied |
| `T1-FC03` | stale member hash, member add/remove, or C1 source/count/byte mismatch during evaluation | T1-FT03 | suppressed / denied |
| `T1-FC04` | absolute/UNC/`..` path, private identity, credential, or unrestricted log text reaches a projection | T1-FT04 | suppressed / denied |
| `T1-FC05` | null/empty/missing/unknown field; unknown enum; incompatible evidence or authority pair; duplicate/misordered/unresolved row | T1-FT05 | suppressed / denied |
| `T1-FC06` | any partition, summary, referenced-bit, one-to-one obligation, or simultaneous conservation mismatch | T1-FT06 | suppressed / denied |
| `T1-FC07` | Contradictory with fewer than two distinct targets is invalid | T1-FT05 | suppressed / denied |
| `T1-FC08` | valid Contradictory with two sorted targets and two evidence refs | T1-FT07 | candidate Rejected; final vector waits for candidate conservation |
| `T1-FC09` | Unavailable with zero or two obligations, unknown source/kind, open-ended selector, or non-positive cap | T1-FT08 | candidate Rejected; final vector waits for candidate conservation |
| `T1-FC10` | Unavailable with exactly one bounded, candidate-local obligation and valid expected kinds | no failure | relationship/obligation conserved one-to-one |
| `T1-FC11` | Present with null target, Absent without exhaustive authority, or C1/FFS claimed as relationship authority | T1-FT05 | suppressed / denied |
| `T1-FC12` | one otherwise canonical subject row uses an unknown `subjectKind` | T1-FT05 | suppressed / denied |
| `T1-FC13` | one schema-valid candidate lacks Controller, action-universe anchor, or attack-universe anchor | T1-FT09 | candidate Rejected; final vector waits for candidate conservation |
| `T1-FC14` | zero evaluated candidates, or one/three evaluated candidates all Rejected | T1-FT10 | diagnostic-only Failed / denied |
| `T1-FC15` | four candidates, skipped ordinal, duplicate/case-conflicting identity, or two Locked | T1-FT11 | suppressed / denied |
| `T1-FC16` | pre-existing temp/output, atomic install failure, or canonical/content-hash mismatch | T1-FT12 | suppressed / denied; pre-existing bytes retained |
| `T1-FC17` | one candidate, all lock predicates true, all formulas true, fresh inputs | T1-FT13 | immutable Passed / exact human confirmation pending |
| `T1-FC18` | three candidates with two Rejected then one Locked, all failure and summary rows conserved | T1-FT13 | immutable Passed with visible rejections / confirmation pending |
| `T1-FC19` | Passed artifact has `consumableForLoCerg1=false`, or Failed artifact has it true | T1-FT05 | suppressed / denied |
| `T1-FC20` | canonical Passed artifact exists but exact human confirmation is absent, stale, or names a different output hash/obligation set | none (external LO gate) | immutable non-authorizing T1-O01 / LO denied |

The matrix must additionally cover every one of the twenty subject kinds, every relationship/query enum, unreferenced evidence retained as `referenced=false`, and raw-content hash versus structured domain-tag digest separation. GREEN requires all formulas to hold simultaneously, every T1-FT01 through T1-FT13 vector to match this table, failed gates to yield zero LO-consumable outputs, and only T1-FT13 to reach exact human confirmation.
...
- `CERG-T1` keeps every repository, source, C1, FFS, and historical-evidence input read-only. Its only writes are creation of the exact T1-T01/T1-O01 parent directories and the non-overwriting atomic T1-T01-to-T1-O01 publication defined above; it must then stop at the exact human confirmation for `LO-CERG1`.
