...
- `CERG-T0` completed at `ordinaryTaskUsed=1/6`; the sole valid CERG-T1A slot is now consumed, so current counters are `ordinaryTaskUsed=2/6`, `LOUsed=0/3`. The historical T1-v1 evaluation and its purported T4 conclusion are both `InvalidatedByContractDefect` and consumed no valid route slot. TO01/TO02/TO03 corrections and the current docs-only TO04 registration consume no additional slot.
...
4. `CERG-T1A` remains consumed exactly once at `ordinaryTaskUsed=2/6`, `LOUsed=0/3`. TO01, TO02, and TO03 are immutable `RejectedByTotalControlAudit` history. The current docs-only correction registers TO04 and stops; after its audit, separately authorized `CERG-T1A4` may change only TG02/TG03, run the twenty-four TO04 fixture classes, commit their exact bytes, publish TO04 without reading C1/FFS candidate evidence or creating T01/O01, and stop for independent TO04 audit.
5. Only after total control approves the exact TO04 SHA and its corrective tool HEAD may separately authorized `CERG-T1B` read the allowed C1/FFS evidence, project the exact candidate members, lock `char_14401`, freeze the eleven discovery obligations, and publish T01/O01. It cannot modify TG01-TG03 or TO01-TO04, completes at `ordinaryTaskUsed=3/6`, `LOUsed=0/3`, and stops before preflight.
