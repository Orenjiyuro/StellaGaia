...
4. `CERG-T1A` remains consumed exactly once at `ordinaryTaskUsed=2/6`, `LOUsed=0/3`. TO01 and TO02 are immutable `RejectedByTotalControlAudit` history. The current docs-only correction registers TO03 and stops; after its audit, separately authorized `CERG-T1A3` may change only TG02/TG03, run the twenty-three TO03 fixture classes, commit their exact bytes, publish TO03 without reading C1/FFS candidate evidence or creating T01/O01, and stop for independent TO03 audit.
