...
| LC-I14 | C4 static-observation package | `Tools/AssetImport/Fixtures/FamilyQualificationGate/valid-c4-static-observation-package.json` | Reviewed fixture-only static-observation producer | C4 | Required exact artifact in Phase A; strict schema is `docs/asset-migration/schemas/c4-static-observation-package.schema.json`; real static observation remains separately unauthorized |
...
- C4 reads C3-O01/O02/O03 and both C3-O04 components, plus LC-I01/I02/I03/I06/I07/I08/I13/I14.
...
LC-I14 top level is exactly `schemaVersion`, `generatedAt`, `snapshotId`, `rows`. Its accepted C4 static-observation row is exactly `assetObjectId`, `familyId`, `checkId`, `outcome`, `reasonCode`, `observedFingerprint`, `failureClasses`, `evidenceKinds`, `evidence`. Rows are unique and Ordinal sorted by `(assetObjectId,checkId)`. `observedFingerprint` is a lowercase 64-hex SHA-256 over the evidence bytes when present. `failureClasses` is an Ordinal set using the frozen common failure-class vocabulary. It is nonempty exactly for Failed and empty for Passed or Unchecked. C4 validates and copies this typed attribution; C6 never parses a failure class from `reasonCode`, `failureAttribution`, a check ID, or free text.

C4 accepts exactly thirteen registered inputs. For each input it requires the registered artifactId/path, exact accepted bytes, and a matching direct-input SHA-256; the LX-HI-14 frame includes artifactId, path, and SHA. The parsed C3-O01/O02/O03, LC-I06 rows, LC-I07 registry, LC-I08 `memberStaticStatus`, and LC-I14 rows must equal their accepted bytes, while the C3 summary must bind the exact C3 output bytes and all snapshot/generation identities must agree. Missing, extra, duplicate, misordered, hash-mismatched, stale, or execution-detached prerequisites fail as `LF-01:C4:DirectInputs` and suppress C4-O01/O02 while retaining only the diagnostic output. LC-I14's Phase A bytes are fixture authority only; this does not authorize asset reads, Unity, extraction, import, or a real static-observation producer.
...
The C0 `authoring-reuse-ledger.schema.json` contract change is complete at schemaVersion `2.0.0`. It now losslessly represents source artifacts, typed family selectors, member identities and bytes, pool isolation, SP-60 coverage, SP-61 capability projection, and both policy fingerprints. The byte-identical positive contract fixture is the C6-O01 fixture; the former LF-19 blocker is closed for fixture-only C6 output generation. Root/G5 schema integration remains a separate R5 Task.
...
The status vocabulary now contains the exact `familyParentStatus` and `memberStaticStatus` dimensions consumed by C3/C4, `representativeAssessment` and `capabilitySuitabilityStatus` consumed by C5, and `authoringPoolStatus` plus `capabilityStatus` consumed immediately by the C6-0 pure decision kernel. Component-local aliases are forbidden.

The C3-2 correction checkpoint closed the previously incomplete C3 assignment/reference behavior: every required non-key support fact must now be Known, the accepted AR-O04 selector set supplies the only legal known-empty `DependencyObjectIds` case, LC-I01 is the dependency reference authority, and all five SP-31 reference kinds participate in distinct Resolved/Missing/Conflict accounting without changing direct family membership. At that checkpoint, the separately reviewed exact-byte object/bytes binding, Ordinal ordering, C5 evidence authority, C6 child conservation, LC-I12, and specification-conflict findings remained prerequisites; the later correction checkpoints and C6-0/C6-1 completion records below close them for the fixture-only path.

The earlier exact-byte/Ordinal correction checkpoint bound policy/evidence objects but did not yet close every base execution artifact. The post-audit C3 correction validates the exact LC-I01-I08/I13 artifactId/path registry, exact bytes and SHA values, bound execution projections, and artifactId-bearing freshness. Its LC-I05 consumer now validates the real AR-S11 envelope, reads `failureAccounting.gateStatus`, requires a complete Passed 5/5/0/0 output generation, binds the exact LC-I01-I04 child-summary bytes, and closes the shared C2/LC-I06 generation identity; simplified, failed, incomplete, detached, stale, or malformed LC-I05 bytes fail as LF-01 diagnostic-only output instead of throwing. The post-audit C4 correction freezes LC-I14 and applies the same model to its exact thirteen-entry registry, including C3 output-generation binding and LF-01 diagnostic-only rejection. The post-audit C5 correction now validates its exact fourteen-entry base registry, execution projections, accepted bytes and direct-input SHA values; binds C3/C4 generation identities and child-summary output hashes; and includes `artifactId` in both base and full input-fingerprint frames. Missing, misordered, detached, or hash-mismatched base authority fails closed as LF-01 with diagnostic-only output, while LC-I09/LC-I10 evidence-authority failures remain LF-15. C6-0 binds every parsed C3-O01/O02/O03, C4-O01/O02, and C5-O01/O02 execution object to the exact bytes named by its direct-input SHA, binds LC-I07/LC-I08/LC-I11/LC-I12, validates the exact 17-entry direct-input set, and validates LC-I12 freshness over the exact 16-entry set excluding itself. Identity, set, subject, and output ordering uses `StringComparer.Ordinal`, enforced by module-AST regression assertions, and C5 risk-variant framing preserves positional `sourceFactKinds` Tuple order.

The C4-to-C6 typed projection checkpoint is complete. Static observations now carry a frozen failure-class set; C4 rejects text attribution and unknown/incorrectly partitioned classes, derives available input kinds only from Known LC-I06 facts and accepted observation evidence kinds, and conserves both projections through check, member, and family identities/outputs. The fixture refresh propagates the changed C4 identities through C5 and the empty LC-I12 input fingerprint without changing the established C5 outcome partitions or launching an executor.

The C6 child-conservation checkpoint is complete as a pure pre-decision kernel. It validates that C3-O01/O02, C4-O01/O02, and C5-O01/O02 expose exactly one mutually consistent row for every C3 family, assigned member, SP-50 risk requirement, and SP-51 capability-suitability requirement. Missing, duplicate, extra, cross-family, cross-route, or requirement/assessment-mismatched rows fail closed as LF-16 or LF-18 and expose no validated subject universe to decision evaluation. This checkpoint applies no C6 Decision Precedence rule, emits no C6-O01 through C6-O05 artifact, and does not authorize C6-0 automatically.

The C6-0 pure decision checkpoint is complete. It consumes the conserved child universes, binds every parsed C3-C5 execution package to its registered exact bytes, verifies the C3 reference generation, consumes exact LC-I07/LC-I08/LC-I11/LC-I12 bytes and the new C6-owned vocabulary, validates LC-I12 attempt identity, repair-rule resolution, expected-change measure, outcome relation, uniqueness, and direct-input freshness, and applies the frozen family-decision precedence without serializing a ledger. The base fixture partitions five families as one NeedsDiagnosis, one RepairOnce, and three RetainForLater; a valid prior attempt removes RepairOnce eligibility, hard-stop precedes repair, complete Audio risk evidence permits UseOriginalAsset, and exact BGM/combat-SFX suitability affects only its own SP-61 capability result. No C6-O01 through C6-O05 artifact, publication, G5 handoff, Unity, or C7/G4 execution is produced.
...
The fixture-only C3-C6 lifecycle path and the C0 authoring reuse ledger v2 contract are implemented through C6-O05 and have passed their independent completion audit. Root summary schema 2.0.0 and the lightweight fixture-only G5 freshness-validating aggregator are also implemented: G5 reads only the Passed C6-O04 among C3-C6 artifacts, validates exact-byte inputs and shared-snapshot conservation, and emits only an in-memory root summary with zero executor/heavy/publication activity. Lifecycle publication, C2 Phase B execution, real assets, extraction, import, and C7/G4 Unity evidence remain separately blocked or unauthorized.
