...
namespace StellaGaia.VASP
{
    public static class VisualAssetSubsetProofRunner
    {
        ...
        private const int VisualAttemptCount = 7;
        private const int ExplicitExcludedCount = 5;
        private const int ShadowedMemberCount = 5;
        ...
        private static readonly string[] VisualBundlePaths =
        {
        ...
        private static readonly string[] ExplicitExcludedPaths =
        {
        ...
        private static string legacySuppressionFailure;

        [RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.BeforeSceneLoad)]
        private static void SuppressLegacyUdcpAutoRun()
        {
            try
            {
                Type legacy = Type.GetType(
                    "StellaGaia.UDCP.DirectCharacterPlayerProofRunner, Assembly-CSharp",
                    false);
                if (legacy == null)
                {
                    throw new InvalidOperationException(
                        "Legacy UDCP runner type was not found.");
                }
                System.Reflection.FieldInfo field = legacy.GetField(
                    "hasRun",
                ...
                {
                    throw new InvalidOperationException(
                        "Legacy UDCP auto-run gate was not found.");
                }
                field.SetValue(null, true);
            }
            catch (Exception exception)
            {
                legacySuppressionFailure = exception.GetType().FullName +
                    ": " + exception.Message;
            }
        }
        ...
        private static void RunProof()
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(legacySuppressionFailure))
                {
                    throw new InvalidOperationException(
                        "Legacy runner suppression failed: " +
                        legacySuppressionFailure);
                }
                ...
                report.nextAction = "AwaitVASPFinalAudit";
                exitCode = 0;
            }
            catch (Exception exception)
            {
        ...
        private static PlayerReport CreateReport()
        {
            ...
            {
                schemaVersion = "visual-subset-proof/1.0.0",
                status = "Running",
                ...
                excludedBundles = new List<ExcludedBundleResult>(),
            ...
            PlayerReport report)
        {
            ...
            if (report.shadowedMemberCount != ShadowedMemberCount)
            {
                throw new InvalidDataException(
                    "Shadowed member conservation failed.");
            }
        ...
        private static void RecordVisualPartition(
            List<EffectiveBundle> effective,
            PlayerReport report)
        {
            var byPath = new Dictionary<string, EffectiveBundle>(
                StringComparer.Ordinal);
            foreach (EffectiveBundle bundle in effective)
            {
                byPath.Add(bundle.relativePath, bundle);
            }
            var partition = new HashSet<string>(StringComparer.Ordinal);
            foreach (string path in VisualBundlePaths)
            {
                if (!byPath.ContainsKey(path) || !partition.Add(path))
                {
                    throw new InvalidDataException(
                        $"Visual partition member is invalid: {path}.");
                }
            }
            foreach (string path in ExplicitExcludedPaths)
            {
                if (!byPath.TryGetValue(path, out EffectiveBundle descriptor) ||
                    !partition.Add(path))
                {
                    throw new InvalidDataException(
                        $"Excluded partition member is invalid: {path}.");
                }
                report.excludedBundles.Add(new ExcludedBundleResult
                {
                    logicalName = descriptor.logicalName,
                    relativePath = descriptor.relativePath,
                    reason = "ScriptBearingOrNonVisualRoute"
                });
            }
            report.visualAttemptCount = VisualBundlePaths.Length;
            report.explicitExcludedCount = report.excludedBundles.Count;
            if (report.visualAttemptCount != VisualAttemptCount ||
                report.explicitExcludedCount != ExplicitExcludedCount ||
                partition.Count != EffectiveBundleCount)
            {
                throw new InvalidDataException(
                    "Effective 7/5 partition conservation failed.");
            }
        }

        private static void AttemptVisualBundles(
            List<EffectiveBundle> effective,
            ...
            PlayerReport report)
        {
            ...
            {
                byPath.Add(descriptor.relativePath, descriptor);
            }
            string firstFailurePath = null;
            foreach (string path in VisualBundlePaths)
            {
                EffectiveBundle descriptor = byPath[path];
                ...
                var result = new BundleResult
                {
                    ...
                    relativePath = descriptor.relativePath,
                    status = "Loading",
                    attemptOrdinal = report.bundleResults.Count + 1
                };
                report.bundleResults.Add(result);
                AssetBundle bundle = null;
                try
                {
                    bundle = AssetBundle.LoadFromFile(descriptor.absolutePath);
                    if (bundle == null)
                    {
                        throw new InvalidDataException(
                ...
                catch (Exception exception)
                {
                    ...
                    result.exceptionMessage =
                        SanitizePortableText(exception.Message, null, null);
                    report.bundleLoadFailureCount++;
                    if (firstFailurePath == null)
                    {
                        firstFailurePath = descriptor.relativePath;
                    }
                    continue;
                }
            }
            if (report.bundleResults.Count != VisualAttemptCount ||
                loaded.Count + report.bundleLoadFailureCount != VisualAttemptCount)
            {
                throw new InvalidDataException(
                    "Visual bundle attempt conservation failed.");
            }
            if (firstFailurePath != null)
            {
                SetContext(
        ...
        private static RenderRig CreateRenderRig(Bounds bounds)
        {
            ...
            var cameraObject = new GameObject("VASP-Player-Camera");
            var lightObject = new GameObject("VASP-Player-Light");
        ...
        private sealed class PlayerReport
        {
            ...
            public int visualAttemptCount;
            public int explicitExcludedCount;
            public int bundleLoadFailureCount;
            public bool consumerValidationStarted;
            public List<MemberResult> memberResults;
            public List<EffectiveBundleResult> effectiveBundles;
            public List<ExcludedBundleResult> excludedBundles;
        ...
        [Serializable]
        private sealed class ExcludedBundleResult
        {
            public string logicalName;
            public string relativePath;
