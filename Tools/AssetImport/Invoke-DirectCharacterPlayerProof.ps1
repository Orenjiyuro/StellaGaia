...
$attemptRoot = Join-Path $worktree 'Extracted\DirectCharacterConsumerProof\char_14401\UDCP-LO2-Player'
$buildLogPath = Join-Path $attemptRoot 'unity-build.log'
$buildResultPath = Join-Path $attemptRoot 'build-result.json'
$playerPath = Join-Path $attemptRoot 'Build\DirectCharacterPlayerProof.exe'
...
$result = [ordered]@{
    schemaVersion = 'player-proof-orchestration/1.0.0'
    status = 'Running'
    stage = 'Preflight'
    processStartCount = 0
...
try {
    ...
    if ([System.IO.Directory]::Exists($attemptRoot) -or
        ...
        throw 'UDCP-LO2-Player attempt root already exists.'
    }
    $active = @(Get-Process -ErrorAction SilentlyContinue | Where-Object {
        $_.ProcessName -eq 'Unity' -or
        $_.ProcessName -eq 'DirectCharacterPlayerProof' -or
    ...
    $buildStartInfo.ArgumentList.Add(
        'StellaGaia.Editor.DirectCharacterPlayerProofBuilder.Build')
    ...
    $playerStartInfo.Environment['STELLAGAIA_UDCP_PLAYER_INPUT_ROOT'] =
        $inputRoot
    $playerStartInfo.Environment['STELLAGAIA_UDCP_PLAYER_OUTPUT_ROOT'] =
        $playerOutputRoot
