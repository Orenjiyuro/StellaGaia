    ...
    foreach($definition in $testDefinitions){
        ...
        try {
            ...
            switch($caseId){
                ...
                'T1A5-TEST26' {
                    $legacy=[pscustomobject][ordered]@{schemaVersion='cerg-lo-cerg1-attempt-state/1.1.0';artifactId='LO-CERG1-A01';attemptCount=[int64]1;status='StartedNoResult'}
                    $legacyBudget=Get-CergR01BudgetSnapshot -Attempt $legacy
                    Assert-CergFixture ($legacyBudget.ordinaryTaskUsed-eq4-and$legacyBudget.ordinaryTaskBudget-eq7-and$legacyBudget.LOUsed-eq1-and$legacyBudget.LOBudget-eq3) 'Legacy R1 budget snapshot was not exactly 4/7,1/3.'

                    $r2=[pscustomobject][ordered]@{schemaVersion='cerg-lo-cerg1-r2-attempt-state/1.0.0';artifactId='LO-CERG1-R2-A01';LOUsedBeforeFirstSourceOpen=[int64]1;LOUsedAfterFirstSourceOpen=[int64]2;status='ArmedNoSourceOpen'}
                    $r2Budget=Get-CergR01BudgetSnapshot -Attempt $r2
                    Assert-CergFixture ($r2Budget.ordinaryTaskUsed-eq5-and$r2Budget.ordinaryTaskBudget-eq7-and$r2Budget.LOUsed-eq2-and$r2Budget.LOBudget-eq3) 'R2 budget snapshot was not exactly 5/7,2/3.'

                    foreach($tamperedValue in @([int64]1,[int64]3)){
                        $tampered=(ConvertTo-CergCanonicalJsonValue $r2)|ConvertFrom-Json -Depth 20 -DateKind String
                        $tampered.LOUsedAfterFirstSourceOpen=$tamperedValue
                        $message='';try{Get-CergR01BudgetSnapshot -Attempt $tampered|Out-Null}catch{$message=$_.Exception.Message}
                        Assert-CergFixture ($message-ceq'R2 attempt LO counter transition is invalid.') "Tampered R2 LO counter $tamperedValue was not rejected exactly."
                    }
                }
                'T1A6-TEST27' {
                    $projectSettings=New-CergUnityGraphFixture (Join-Path $caseRoot 'ProjectSettingsExcluded')
                    $settingsLeaf=Join-Path $projectSettings.OutputRoot 'ExportedProject\ProjectSettings\EditorSettings.asset'
                    Write-CergFixtureBytes $settingsLeaf $script:CergUtf8NoBom.GetBytes("EditorSettings:`n  m_SerializationMode: 2`n")
                    $settingsResult=Invoke-CergGraphFixture $projectSettings
                    Assert-CergFixture ($settingsResult.status-ceq'Closed'-and$settingsResult.consumableForT2-and@($settingsResult.outputMembers|Where-Object portableRelativePath -CEQ 'ExportedProject/ProjectSettings/EditorSettings.asset').Count-eq0-and@($settingsResult.diagnostics).Count-eq0) 'ProjectSettings infrastructure asset was not excluded from strict resource discovery.'

                    $missingMeta=New-CergUnityGraphFixture (Join-Path $caseRoot 'AssetsMissingMeta')
                    $assetLeaf=Join-Path $missingMeta.OutputRoot 'ExportedProject\Assets\Missing.asset'
                    Write-CergFixtureBytes $assetLeaf $script:CergUtf8NoBom.GetBytes("--- !u!114 &1`nMonoBehaviour:`n  m_Name: MissingMeta`n")
