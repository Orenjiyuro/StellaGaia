...
function Invoke-C2CanonicalPartitions {
    ...
    $conservationInvalid=$expected.Count-ne$actual.Count-or(@(Compare-Object $expected $actual -SyncWindow 0).Count)-gt0-or$proposalIds.Count-ne$proposals.Count
    ...
    $valid=[Collections.Generic.List[object]]::new();if(-not$conservationInvalid){foreach($p in $proposals){$ok=$false;try{$names=@($p.PSObject.Properties.Name)-join',';$facts=@($p.memberFacts);$members=@(Get-C2OrdinalUnique @($p.memberObjectIds));$missing=@(Get-C2OrdinalUnique @($p.missingMemberObjectIds));$factIds=@(Get-C2OrdinalUnique @($facts.assetObjectId));$union=@(Get-C2OrdinalUnique @($factIds+$missing));$factShape=@($facts|Where-Object{(@($_.PSObject.Properties.Name)-join',')-cne'assetObjectId,memberPlatform,contentFingerprint'-or$_.memberPlatform-cnotin@('Pc','Android','Unknown')-or$_.contentFingerprint-cnotmatch'^[0-9a-f]{64}$'}).Count-eq0;$ok=$names-ceq'proposalId,observationId,canonicalEvidenceId,proposedCanonicalAssetId,matchStatus,platformScope,equivalenceFingerprint,memberObjectIds,memberFacts,missingMemberObjectIds,evidence'-and$factShape-and$members.Count-eq@($p.memberObjectIds).Count-and$missing.Count-eq@($p.missingMemberObjectIds).Count-and@($factIds|Where-Object{$_-cin$missing}).Count-eq0-and($union-join',')-ceq($members-join',')-and$p.proposalId-ceq(Get-C2CanonicalProposalId $p)}catch{$ok=$false};if($ok){$valid.Add($p)}else{$conservationInvalid=$true;break}}}
    ...
    if(-not$conservationInvalid){
        ...
        $processed=[Collections.Generic.HashSet[string]]::new($script:Ordinal);foreach($id in @($byId.Keys)){if($processed.Contains([string]$id)){continue};$component=[Collections.Generic.List[string]]::new();$component.Add([string]$id);for($ci=0;$ci-lt$component.Count;$ci++){$current=$component[$ci];$currentMembers=@(Get-C2OrdinalUnique @($byId[$current].memberObjectIds));foreach($member in $currentMembers){foreach($other in @($objectToIds[$member])){if(-not$component.Contains($other)){$component.Add($other)}}}};foreach($componentId in $component){$null=$processed.Add($componentId)};$rows=@($component|ForEach-Object{$byId[$_]});$members=@(Get-C2OrdinalUnique @($rows.memberObjectIds));$observations=@(Get-C2OrdinalUnique @($rows.observationId));$proposed=@(Get-C2OrdinalUnique @($rows.proposedCanonicalAssetId));$fields=[Collections.Generic.List[string]]::new();$first=$rows[0]
            ...
            $derivedId=Get-C2CanonicalGroupId $first.matchStatus $derivedScope $first.equivalenceFingerprint $members;if($first.platformScope-cne$derivedScope){$fields.Add('platformScope')};if($first.proposedCanonicalAssetId-cne$derivedId){$fields.Add('proposedCanonicalAssetId')}
            if($first.matchStatus-ceq'ExactDuplicate'-and($members.Count-lt2-or$contents.Count-ne1)){$fields.Add('contentFingerprint')};if($first.matchStatus-ceq'ConfirmedVariant'){$variant=Get-C2VariantEquivalenceFingerprint $contents;if($platforms.Count-lt2){$fields.Add('memberPlatform')};if($contents.Count-lt2){$fields.Add('contentFingerprint')};if($first.equivalenceFingerprint-cne$variant){$fields.Add('equivalenceFingerprint')}}}
