...
function Get-CergR2ConfirmationId {
    param([string]$ContractHeadCommit,[string]$F01Sha256,[string]$F01CheckRunId,[string]$MemberSetFingerprint,[string]$P02Sha256,[string]$RunnerImplementationId)
    'R2CONF-'+(Get-CergStructuredSha256 'cerg-r2/p02-confirmation-id/1' @($ContractHeadCommit,$F01Sha256,$F01CheckRunId,$MemberSetFingerprint,$P02Sha256,$RunnerImplementationId))
}
...
function Assert-CergR2APlusReadiness {
    ...
    if($Readiness.schemaVersion-cne'cerg-r2-aplus-readiness/1.4.0'-or$Readiness.artifactId-cne'R2-TO07'-or$Readiness.contractHeadCommit-cne$ContractHeadCommit-or$Readiness.status-cne'Green'-or@($Readiness.failures).Count-ne0-or$Readiness.nextAction-cne'AwaitTotalControlAuditBeforeR2P02Generation'){throw'AtomicHandoffFailure: A+ readiness fixed state invalid.'}
    ...
    $aplus=@('R2A-CE01-ConfirmationMismatch','R2A-CE02-SourceDrift','R2A-CE03-ExpiredBeforeOpen','R2A-CE04-ForeignProcess','R2A-CE05-A01WithoutFirstOpen','R2A-CE06-HeadDomainSplice','R2A-CE07-OrdinalOrdering','R2A-CE08-ImplementationBindings','R2A-CE09-RuntimeSelfConsistentSplice','R2A-CE10-FakeAssetRipperExecutable','R2A-SP01-AtomicHandoff')
    ...
    if($actualTests.Count-ne55-or(@($actualTests|ForEach-Object testId)-join'|')-cne($expectedTests-join'|')-or@($actualTests|Where-Object{$_.status-cne'Passed'-or[string]::IsNullOrWhiteSpace([string]$_.evidenceLocator)}).Count-ne0){throw'AtomicHandoffFailure: A+ readiness test matrix invalid.'}
    $expectedPaths=@('Tools/AssetImport/Invoke-CergLo1ExactStaging.ps1','Tools/AssetImport/Invoke-CergLo1R2AtomicHandoff.ps1','Tools/AssetImport/New-CergLo1Preflight.ps1','Tools/AssetImport/New-CergLo1R2Freshness.ps1','Tools/AssetImport/New-CergLo1ResultGraph.ps1','Tools/AssetImport/Test-CergLo1Pipeline.ps1','Tools/AssetImport/Test-CergLo1R2AtomicHandoff.ps1','Tools/AssetImport/Test-CergLo1R2Freshness.ps1','Tools/AssetImport/Test-CergLo1R2FreshnessPipeline.ps1','Tools/AssetImport/Invoke-AssetRipperFolderExport.ps1');$actualPaths=@($Readiness.toolRows|ForEach-Object portableRelativePath);$expectedSet=[string[]]@($expectedPaths);[Array]::Sort($expectedSet,[StringComparer]::Ordinal);$sortedPaths=[string[]]@($actualPaths);[Array]::Sort($sortedPaths,[StringComparer]::Ordinal)
    $actualArtifactIds=[string[]]@($Readiness.toolRows|ForEach-Object artifactId);$sortedArtifactIds=[string[]]@($actualArtifactIds);[Array]::Sort($sortedArtifactIds,[StringComparer]::Ordinal);$expectedArtifactIds=@(1..10|ForEach-Object{'R2-A-T{0:D2}'-f$_})
    if($actualPaths.Count-ne10-or($sortedPaths-join'|')-cne($expectedSet-join'|')-or($actualArtifactIds-join'|')-cne($sortedArtifactIds-join'|')-or($actualArtifactIds-join'|')-cne($expectedArtifactIds-join'|')){throw'AtomicHandoffFailure: A+ readiness tool set/order invalid.'}
...
function Invoke-CergLo1R2AtomicHandoff {
    ...
    $null=Assert-CergLo1PreflightConsumerContract -Candidate $candidate -Preflight $p01 -Freshness $f01 -Readiness $readiness -FreshnessEvidencePath $ConfirmedF01Path -StagingInventoryTemporaryPath $StagingInventoryTemporaryPath -StagingInventoryPath $StagingInventoryPath
    if($p01.schemaVersion-cne'cerg-lo-cerg1-preflight/1.6.0'-or$p01.artifactId-cne'LO-CERG1-P02'){throw'AtomicHandoffFailure: only P02 v1.6 is consumable.'}
