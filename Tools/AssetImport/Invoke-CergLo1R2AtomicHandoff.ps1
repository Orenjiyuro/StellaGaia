...
function Assert-CergR2APlusReadiness {
    ...
    $aplus=@('R2A-CE01-ConfirmationMismatch','R2A-CE02-SourceDrift','R2A-CE03-ExpiredBeforeOpen','R2A-CE04-ForeignProcess','R2A-CE05-A01WithoutFirstOpen','R2A-CE06-HeadDomainSplice','R2A-CE07-OrdinalOrdering','R2A-CE08-ImplementationBindings','R2A-SP01-AtomicHandoff')
    ...
    if($actualTests.Count-ne53-or(@($actualTests|ForEach-Object testId)-join'|')-cne($expectedTests-join'|')-or@($actualTests|Where-Object{$_.status-cne'Passed'-or[string]::IsNullOrWhiteSpace([string]$_.evidenceLocator)}).Count-ne0){throw'AtomicHandoffFailure: A+ readiness test matrix invalid.'}
