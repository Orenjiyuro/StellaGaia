...
function Assert-CergR2APlusReadiness {
    ...
    if($Readiness.schemaVersion-cne'cerg-r2-aplus-readiness/1.0.0'-or$Readiness.artifactId-cne'R2-TO03'-or$Readiness.contractHeadCommit-cne$ContractHeadCommit-or$Readiness.status-cne'Green'-or@($Readiness.failures).Count-ne0-or$Readiness.nextAction-cne'AwaitTotalControlAuditBeforeR2P01'){throw'AtomicHandoffFailure: A+ readiness fixed state invalid.'}
    ...
    $aplus=@('R2A-CE01-ConfirmationMismatch','R2A-CE02-SourceDrift','R2A-CE03-ExpiredBeforeOpen','R2A-CE04-ForeignProcess','R2A-CE05-A01WithoutFirstOpen','R2A-SP01-AtomicHandoff')
    ...
    if($actualTests.Count-ne50-or(@($actualTests|ForEach-Object testId)-join'|')-cne($expectedTests-join'|')-or@($actualTests|Where-Object{$_.status-cne'Passed'-or[string]::IsNullOrWhiteSpace([string]$_.evidenceLocator)}).Count-ne0){throw'AtomicHandoffFailure: A+ readiness test matrix invalid.'}
