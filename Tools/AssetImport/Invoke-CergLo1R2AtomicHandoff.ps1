...
function Assert-CergR2APlusReadiness {
    ...
    $aplus=@('R2A-CE01-ConfirmationMismatch','R2A-CE02-SourceDrift','R2A-CE03-ExpiredBeforeOpen','R2A-CE04-ForeignProcess','R2A-CE05-A01WithoutFirstOpen','R2A-CE06-HeadDomainSplice','R2A-CE07-OrdinalOrdering','R2A-SP01-AtomicHandoff')
    ...
    if($actualTests.Count-ne52-or(@($actualTests|ForEach-Object testId)-join'|')-cne($expectedTests-join'|')-or@($actualTests|Where-Object{$_.status-cne'Passed'-or[string]::IsNullOrWhiteSpace([string]$_.evidenceLocator)}).Count-ne0){throw'AtomicHandoffFailure: A+ readiness test matrix invalid.'}
    ...
    $actualArtifactIds=[string[]]@($Readiness.toolRows|ForEach-Object artifactId);$sortedArtifactIds=[string[]]@($actualArtifactIds);[Array]::Sort($sortedArtifactIds,[StringComparer]::Ordinal);$expectedArtifactIds=@(1..9|ForEach-Object{'R2-A-T{0:D2}'-f$_})
    if($actualPaths.Count-ne9-or($sortedPaths-join'|')-cne($expectedPaths-join'|')-or($actualArtifactIds-join'|')-cne($sortedArtifactIds-join'|')-or($actualArtifactIds-join'|')-cne($expectedArtifactIds-join'|')){throw'AtomicHandoffFailure: A+ readiness tool set/order invalid.'}
