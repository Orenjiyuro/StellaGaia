...
if($Case -ceq 'CanonicalPartitions'){
    ...
    function Clone($x){$x|ConvertTo-Json -Depth 30|ConvertFrom-Json -Depth 30};function RunCanonical($x){$loadedModule.Invoke({param($v)Invoke-C2CanonicalPartitions $v},@($x))[0]}
