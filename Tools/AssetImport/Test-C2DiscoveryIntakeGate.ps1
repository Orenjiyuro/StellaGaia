[CmdletBinding()]
param(
    [ValidateSet('Pure','GitAdapter','Integration','FailureState','ValidatorMutations','FileFixtureIntake','FilePartitions','ObjectObservationPartitions','ObjectMergeProjection','ConfigurationPartitions','CanonicalPartitions','DispatchPartitions','LaneFactProjection','InputAccounting','OutputSerialization','PublicationTransaction','PublicationIntegration')]
    [string]$Case = 'Pure'
)
...
function Invoke-LaneFactProjectionTest {
    $repositoryRoot=(Resolve-Path (Join-Path $PSScriptRoot '../..')).Path;$utf8=[Text.UTF8Encoding]::new($false)
    function CanonicalBytes($value){$text=[string]@($loadedModule.Invoke({param($x)ConvertTo-C2CanonicalJson $x},@($value)))[0];$utf8.GetBytes($text)}
    function CloneLane($value){$value|ConvertTo-Json -Depth 100|ConvertFrom-Json -Depth 100 -DateKind String}
    function RunLane([byte[]]$dispatch,[byte[]]$summary,[byte[]]$schema){try{@($loadedModule.Invoke({param($d,$s,$c)Invoke-C2TypedLaneFactProjection -DispatchBytes $d -SummaryBytes $s -SchemaBytes $c},@($dispatch,$summary,$schema)))[0]}catch{throw "$($_.Exception.Message) $($_.ScriptStackTrace)"}}
    $generatedAt='2026-07-15T12:30:00Z';$snapshotId='snapshot-pc-install-001';$inputFingerprint='1'*64;$discoveryInputFingerprint='2'*64;$artifactFingerprint='3'*64;$evidence='Tools/AssetImport/Fixtures/DiscoveryGate/object-observations.json';$tool='ToolA:version=1.0.0'
    $rows=[Collections.Generic.List[object]]::new();$lanes=@('Audio','Environment','Actor','UI','Effects')
    for($i=0;$i-lt$lanes.Count;$i++){$id='sha256:'+(([char](97+$i)).ToString()*64);$canonicalChar=@('f','1','2','3','4')[$i];$canonicalId='canonical-sha256:'+($canonicalChar*64);$selectors=[Collections.Generic.List[object]]::new();$selectors.Add([pscustomobject][ordered]@{kind='CanonicalAssetId';value=$canonicalId});$selectors.Add([pscustomobject][ordered]@{kind='ClassId';value=[string](100+$i)});if($i-in@(1,2,4)){$selectors.Add([pscustomobject][ordered]@{kind='DependencyObjectId';value='sha256:'+([string]($i+1)*64)});if($i-eq2){$selectors.Add([pscustomobject][ordered]@{kind='DependencyObjectId';value='sha256:'+('9'*64)})}};$selectors.Add([pscustomobject][ordered]@{kind='ObjectType';value=@('AudioClip','Scene','Avatar','Sprite','ParticleSystem')[$i]});$selectors.Add([pscustomobject][ordered]@{kind='PlatformVariant';value='Pc'});$selectors.Add([pscustomobject][ordered]@{kind='ToolObservation';value=$tool});$rows.Add([pscustomobject][ordered]@{assetObjectId=$id;canonicalAssetId=$canonicalId;sourceId='pc-install-primary';familyLane=$lanes[$i];memberSelectorInputs=[object[]]$selectors;configurationCandidateId=$null;dispatchStatus='Assigned';evidence=@($evidence)})}
    $rows.Add([pscustomobject][ordered]@{assetObjectId='sha256:'+(('e'*63)+'f');canonicalAssetId='canonical-sha256:'+(('e'*63)+'f');sourceId='pc-install-primary';familyLane='Unassigned';memberSelectorInputs=@([pscustomobject][ordered]@{kind='CanonicalAssetId';value='canonical-sha256:'+(('e'*63)+'f')});configurationCandidateId=$null;dispatchStatus='RetainedForDiagnosis';evidence=@($evidence)})
    $rows.Add([pscustomobject][ordered]@{assetObjectId='sha256:'+('f'*64);canonicalAssetId='canonical-sha256:'+('f'*64);sourceId='pc-install-primary';familyLane='Unassigned';memberSelectorInputs=@([pscustomobject][ordered]@{kind='CanonicalAssetId';value='canonical-sha256:'+('f'*64)});configurationCandidateId='config-sha256:'+('9'*64);dispatchStatus='ConfigurationOnly';evidence=@($evidence)})
    $dispatch=[pscustomobject][ordered]@{schemaVersion='1.0.0';generatedAt=$generatedAt;snapshotId=$snapshotId;inputFingerprint=$inputFingerprint;discoveryInputFingerprint=$discoveryInputFingerprint;sourceLedgerPath='Tools/AssetImport/Fixtures/SourceCorpusGate/valid-c1-source-corpus-ledger.json';rows=[object[]]$rows}
    $summary=[pscustomobject][ordered]@{schemaVersion='1.0.0';identity=[pscustomobject][ordered]@{generatedAt=$generatedAt;snapshotId=$snapshotId;inputFingerprint=$inputFingerprint;ledgerInputFingerprint='4'*64;discoveryInputFingerprint=$discoveryInputFingerprint;discoveryArtifactFingerprint=$artifactFingerprint};provenance=[pscustomobject][ordered]@{toolVersions=@();operationIdentity='C2.DiscoveryCoverage.FixtureValidation'};directEvidence=[pscustomobject][ordered]@{discoveryInputs=@();directChildSummaries=@();directChildReports=@()};coverage=[pscustomobject][ordered]@{files=[pscustomobject]@{};containers=[pscustomobject]@{};objects=[pscustomobject]@{};configuration=[pscustomobject]@{};canonical=[pscustomobject]@{};dispatch=[pscustomobject]@{}};failureAccounting=[pscustomobject][ordered]@{gateStatus='Passed'};decision=[pscustomobject][ordered]@{failureAttribution='None';nextAllowedAction='Project LC-I06 fixture.'}}
    $dispatchBytes=CanonicalBytes $dispatch;$summaryBytes=CanonicalBytes $summary;$schemaBytes=[IO.File]::ReadAllBytes((Join-Path $repositoryRoot 'docs/asset-migration/schemas/c2-lane-fact-package.schema.json'));$result=RunLane $dispatchBytes $summaryBytes $schemaBytes
    if($result.gateStatus-cne'Passed'){throw "LC-I06 positive projection failed: $($result.inputFailures.subjectId)/$($result.inputFailures.reasonCode)"}
    if((@($result.PSObject.Properties.Name)-join',')-cne'gateStatus,package,subjectAccounting,inputFailures,outputsSuppressed'-or$result.outputsSuppressed-or$result.inputFailures.Count-or$result.package.rows.Count-ne23-or$result.subjectAccounting.Count-ne7){throw "LC-I06 positive result shape/count failed: facts=$($result.package.rows.Count) accounting=$($result.subjectAccounting.Count)"}
    if(@($result.subjectAccounting|Where-Object terminalStatus -ceq AssignedFactsProjected).Count-ne5-or@($result.subjectAccounting|Where-Object terminalStatus -ceq RetainedNoLaneFacts).Count-ne1-or@($result.subjectAccounting|Where-Object terminalStatus -ceq ConfigurationNoLaneFacts).Count-ne1){throw 'LC-I06 terminal subject accounting failed.'}
    if(@($result.package.rows|Where-Object factKind -ceq DependencyObjectIds).Count-ne3-or@($result.package.rows|Where-Object factStatus -cne Known).Count-or(@($result.package.rows.lane|Sort-Object -Unique)-join',')-cne'Actor,Audio,Effects,Environment,UI'){throw 'LC-I06 fact mapping failed.'}
    $packageText=[string]@($loadedModule.Invoke({param($x)ConvertTo-C2CanonicalJson $x},@($result.package)))[0]
    $fixtureDispatch=CloneLane $dispatch;$fixtureDispatch.rows=@($fixtureDispatch.rows[0],$fixtureDispatch.rows[5],$fixtureDispatch.rows[6]);$fixtureResult=RunLane (CanonicalBytes $fixtureDispatch) $summaryBytes $schemaBytes;if($fixtureResult.gateStatus-cne'Passed'-or$fixtureResult.package.rows.Count-ne4-or$fixtureResult.subjectAccounting.Count-ne3-or$fixtureResult.package.inputFingerprint-ceq$result.package.inputFingerprint){throw 'LC-I06 fixture projection/dispatch fingerprint binding failed.'}
    $fixtureText=[string]@($loadedModule.Invoke({param($x)ConvertTo-C2CanonicalJson $x},@($fixtureResult.package)))[0];$expectedPath=Join-Path $repositoryRoot 'Tools/AssetImport/Fixtures/FamilyQualificationGate/valid-c2-lane-fact-package.json';if(-not[IO.File]::Exists($expectedPath)){throw 'LC-I06 expected fixture missing.'};if($fixtureText-cne[IO.File]::ReadAllText($expectedPath,$utf8)){throw 'LC-I06 expected fixture bytes mismatch.'}
    $repeat=RunLane $dispatchBytes $summaryBytes $schemaBytes;$repeatText=[string]@($loadedModule.Invoke({param($x)ConvertTo-C2CanonicalJson $x},@($repeat.package)))[0];if($repeatText-cne$packageText){throw 'LC-I06 repeated-run determinism failed.'}
    $schemaMutation=[byte[]]($schemaBytes+[byte[]]@(10));$schemaChanged=RunLane $dispatchBytes $summaryBytes $schemaMutation;if($schemaChanged.gateStatus-cne'Passed'-or$schemaChanged.package.factContractFingerprint-ceq$result.package.factContractFingerprint-or$schemaChanged.package.inputFingerprint-ceq$result.package.inputFingerprint){throw 'LC-I06 schema fingerprint binding failed.'}
    $summaryMutation=CloneLane $summary;$summaryMutation.identity.discoveryArtifactFingerprint='5'*64;$generationChanged=RunLane $dispatchBytes (CanonicalBytes $summaryMutation) $schemaBytes;if($generationChanged.gateStatus-cne'Passed'-or$generationChanged.package.c2GenerationFingerprint-cne('5'*64)-or$generationChanged.package.inputFingerprint-ceq$result.package.inputFingerprint){throw 'LC-I06 generation fingerprint binding failed.'}
    $missing=CloneLane $dispatch;$missing.rows[0].memberSelectorInputs=@($missing.rows[0].memberSelectorInputs|Where-Object kind -cne ObjectType);$failed=RunLane (CanonicalBytes $missing) $summaryBytes $schemaBytes;if($failed.gateStatus-cne'Failed'-or$null-ne$failed.package-or-not$failed.outputsSuppressed-or$failed.subjectAccounting.Count-or$failed.inputFailures[0].reasonCode-cne'ConservationMismatch'){throw 'LC-I06 missing-selector fail-closed failed.'}
    $noncanonical=CloneLane $dispatch;@($noncanonical.rows[0].memberSelectorInputs|Where-Object kind -ceq ClassId)[0].value='01';$failedClass=RunLane (CanonicalBytes $noncanonical) $summaryBytes $schemaBytes;if($failedClass.gateStatus-cne'Failed'-or$failedClass.inputFailures[0].reasonCode-cne'InvalidSchema'){throw 'LC-I06 ClassId canonicality failed.'}
    $permuted=CloneLane $dispatch;$swap=$permuted.rows[0];$permuted.rows[0]=$permuted.rows[1];$permuted.rows[1]=$swap;$failedOrder=RunLane (CanonicalBytes $permuted) $summaryBytes $schemaBytes;if($failedOrder.gateStatus-cne'Failed'-or$failedOrder.inputFailures[0].reasonCode-cne'ConservationMismatch'){throw 'LC-I06 input-order fail-closed failed.'}
    if([IO.Directory]::Exists((Join-Path $repositoryRoot 'Temp/C2DiscoveryPublication'))){throw 'LC-I06 projection touched publication state.'}
    'status=Passed';'dispatchSubjectCount=7';'projectedAssignedObjectCount=5';'retainedNoLaneFactObjectCount=1';'configurationNoLaneFactObjectCount=1';'projectedFactCount=23';'publicationWriteCount=0'
}
if($Case -ceq 'LaneFactProjection'){Invoke-LaneFactProjectionTest;return}

if($Case -ceq 'PublicationIntegration'){
    $repositoryRoot=(Resolve-Path (Join-Path $PSScriptRoot '../..')).Path
    $paths=@('Tools/AssetImport/Fixtures/DiscoveryGate/valid-c2-source-corpus-ledger.json','Tools/AssetImport/Fixtures/DiscoveryGate/valid-resolved-configuration-package.json','Tools/AssetImport/Fixtures/DiscoveryGate/valid-canonical-group-package.json','Tools/AssetImport/Fixtures/DiscoveryGate/valid-object-dispatch.json','Tools/AssetImport/Fixtures/DiscoveryGate/valid-discovery-report.md','Tools/AssetImport/Fixtures/DiscoveryGate/valid-discovery-evidence.json','Tools/AssetImport/Fixtures/DiscoveryGate/c0-contract-change-request.json','Tools/AssetImport/Fixtures/DiscoveryGate/valid-discovery-summary.json')
    function Invoke-PublicationIntegrationVector([string]$root,[string[]]$registeredPaths){
        $absolute=@($registeredPaths|ForEach-Object{[IO.Path]::Combine($root,$_.Replace('/',[IO.Path]::DirectorySeparatorChar))});$operational=[IO.Path]::Combine($root,'Temp','C2DiscoveryPublication');if(@($absolute|Where-Object{[IO.File]::Exists($_)}).Count-or[IO.Directory]::Exists($operational)){throw 'PublicationIntegration requires all registered outputs and operational state to be absent.'}
        try{
            $first=Invoke-C2DiscoveryIntakeGate -RepositoryRoot $root;$firstHashes=@($absolute|ForEach-Object{if([IO.File]::Exists($_)){[Convert]::ToHexString([Security.Cryptography.SHA256]::HashData([IO.File]::ReadAllBytes($_))).ToLowerInvariant()}else{$null}})
            $second=Invoke-C2DiscoveryIntakeGate -RepositoryRoot $root;$secondHashes=@($absolute|ForEach-Object{if([IO.File]::Exists($_)){[Convert]::ToHexString([Security.Cryptography.SHA256]::HashData([IO.File]::ReadAllBytes($_))).ToLowerInvariant()}else{$null}})
            $operationEntries=if([IO.Directory]::Exists($operational)){@([IO.Directory]::GetFileSystemEntries($operational)|ForEach-Object{[IO.Path]::GetFileName($_)})}else{@()}
            [pscustomobject][ordered]@{first=$first;second=$second;firstStates=@($firstHashes|ForEach-Object{$null-ne$_});secondStates=@($secondHashes|ForEach-Object{$null-ne$_});sameDiagnosticBytes=(@(4..7|Where-Object{$firstHashes[$_]-cne$secondHashes[$_]}).Count-eq0);operationEntries=$operationEntries}
        }finally{foreach($path in $absolute){if([IO.File]::Exists($path)){[IO.File]::Delete($path)}};if([IO.Directory]::Exists($operational)){[IO.Directory]::Delete($operational,$true)}}
    }
    $snapshot=Invoke-PublicationIntegrationVector $repositoryRoot $paths
    foreach($run in @($snapshot.first,$snapshot.second)){if($run.O2.status-cne'Failed'-or$run.O2.inputSubjectCount-ne24-or$run.O2.acceptedInputSubjectCount-ne21-or$run.O2.inputFailureCount-ne2-or$run.O2.excludedInputSubjectCount-ne1-or$run.O2.gitInspectionProcessCount-ne7-or$run.O2.headStable-cne$true-or$run.Publication.status-cne'Committed'-or$run.Publication.outputAccounting.projectedOutputCount-ne1-or$run.Publication.outputAccounting.outputFailureCount-ne4-or$run.ConsumerValidation.status-cne'Rejected'-or$run.ConsumerValidation.reason-cne'GateFailed'){throw 'public publication integration result invalid.'}}
    if(($snapshot.firstStates-join',')-cne'False,False,False,False,True,True,True,True'-or($snapshot.secondStates-join',')-cne'False,False,False,False,True,True,True,True'-or($snapshot.operationEntries-join',')-cne'publication.lock'){throw 'public publication state/idempotence invalid.'}
    $consumerVectors=@{};foreach($name in @('Passed','Failed','Partial','HashMismatch')){$consumerVectors[$name]=$loadedModule.Invoke({param($root,$vector)Test-C2PublishedGenerationVector $root $vector},@($repositoryRoot,$name))[0]};if($consumerVectors.Passed.status-cne'Accepted'-or$consumerVectors.Failed.reason-cne'GateFailed'-or$consumerVectors.Partial.reason-cne'PartialStaleOrHashMismatch'-or$consumerVectors.HashMismatch.reason-cne'PartialStaleOrHashMismatch'){throw 'locked consumer validation vectors failed.'}
    'status=Passed';'publicationRuns=2';'gitInspectionProcessCountPerRun=7';'publishedStates=False,False,False,False,True,True,True,True';'consumerPassed=Accepted';'consumerFailed=Rejected:GateFailed';'consumerPartial=Rejected:PartialStaleOrHashMismatch';'consumerHashMismatch=Rejected:PartialStaleOrHashMismatch';return
}
...
if($Case -ceq 'OutputSerialization'){
    ...
    $generatedAt='2026-07-15T00:00:00Z';$snapshotId='snapshot-pc-install-001';$inputFingerprint=('a'*64-join'');$ledgerFingerprint=('d'*64-join'');$discoveryFingerprint='96b09428bc1f08b74a85c5b7fb489b4a2fb8242cc3e1a7623b6f855e15dee56b'
    ...
    function New-Contract($fingerprint){[pscustomobject][ordered]@{schemaVersion='1.0.0';generatedAt=$generatedAt;requestId='C0ContractChange:C2StructuredCoverage:1.0.0';reason='Root gate summary schema 2.0.0 losslessly represents mandatory C2 structured coverage partitions.';nextAllowedAction='Provide current C2 outputs and C6-O04 to the separately authorized G5 aggregator.';inputFingerprint=$inputFingerprint;discoveryInputFingerprint=$fingerprint;status='Resolved';missingProjectionFields=@();evidence=@('Tools/AssetImport/Fixtures/DiscoveryGate/valid-discovery-summary.json','docs/asset-migration/schemas/root-gate-summary.schema.json')}}
    ...
    $passedDecision=[pscustomobject][ordered]@{failureAttribution='None; C2 registry-derived gates passed.';nextAllowedAction='Provide current C2 outputs and C6-O04 to the separately authorized G5 aggregator.'}
    ...
    if($passed.transactionId-cne'publication-sha256:452eda2d02f0f2c85d6aadd847fbdee8fe9dccf9edcd1506344728c6a813be32'){throw "Passed HI-16 mismatch: $($passed.transactionId)"}
    if($passed.discoveryArtifactFingerprint-cne'3ddf0cf737027f0074e545bb65307565652ad6b0a757a498e45c1ab4b3101d13'-or$passed.diagnosticBundleFingerprint-cne'873a9472f02472f4dca32d822c3a17d4ecf1114ee54122ff6e0d9fb44c002f16'){throw "Passed HI-13c/HI-14 literal mismatch: $($passed.discoveryArtifactFingerprint),$($passed.diagnosticBundleFingerprint)"}
    if(($passed.artifacts.sha256-join',')-cne'e2f3cb01cd005a7f046d4cdd61a6df873d3406e0d40f0695792d71d04f9f0441,a26d43c648ac1a11d19ed0ee4d43afcde6266108622eca8745110f2a12054f7f,bcc8c280902dba1a50c172c7476f797f72e54c2c6b5d3a2765c7dc1e192e10e8,ff452203269549f18b8699c568d3d4f6ad9cd9b7e57a497f0fee9a492bf93d54,f84e3931dffed0b7641181858c274195503f6ecce562f77eb816c8c9c5e3aa1b,f29b5139213b394fd065fdb73a70caf9f1c6d753d15f1d93a116651a9e51a660,d1331b144c1b54f2f8f0cba4e298b01ec55059a654fd2e47284edd8d7d7e5f26,f36667d6ee15a38071cd1eb0bdebcc3c93aebfa26020db5d10bad92e2eb3c201'){throw "Passed CT-15 literal SHA set mismatch: $($passed.artifacts.sha256-join',')"}
    ...
    if($failed.discoveryArtifactFingerprint-cne'3d16b22ec1b6b699d661b157aa099837d63e6595d02cdd5833a41d3c455848bc'-or$failed.diagnosticBundleFingerprint-cne'58ea4bb11ba9aba3eca4436e08640e2491bbc4f37c17aeeb64f3a728a2082d88'){throw "Failed HI-13c/HI-14 literal mismatch: $($failed.discoveryArtifactFingerprint),$($failed.diagnosticBundleFingerprint)"}
    if(($failed.artifacts.sha256-join',')-cne',,,,36a044640c4f55b3fb9aa4285d523ca812986049d33816b68d0c770a875d08e3,946ee75054fe711a55f15920ff03283730809a5d3b11c2d0db363f6b5b22bbb8,8292413d28adbb246774d842dde38c4fd734478d65281ac8f360d856eefa4a69,fea728b4cf86aa5ca5a68e424438244b95897b92ec37d1a7306007da9fe70e22'){throw "Failed CT-15 literal SHA set mismatch: $($failed.artifacts.sha256-join',')"}
    if($failed.artifacts[4].text-cnotmatch'(?m)^discoveryInputFingerprint: null$'){throw'Failed report null projection failed.'}
    $passedReport="# C2 Discovery Gate Report`nschemaVersion: 1.0.0`ngeneratedAt: 2026-07-15T00:00:00Z`nsnapshotId: snapshot-pc-install-001`noperationIdentity: C2.DiscoveryCoverage.FixtureValidation`ninputFingerprint: aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa`nledgerInputFingerprint: dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd`ndiscoveryInputFingerprint: 96b09428bc1f08b74a85c5b7fb489b4a2fb8242cc3e1a7623b6f855e15dee56b`ngateStatus: Passed`ninputSubjectCount: 21`nnotEvaluatedInputSubjectCount: 0`ninputFailureCount: 0`noutputCandidateCount: 5`nprojectedOutputCount: 5`noutputFailureCount: 0`nissueCount: 0`nfailureAttribution: None; C2 registry-derived gates passed.`nnextAllowedAction: Provide current C2 outputs and C6-O04 to the separately authorized G5 aggregator.`n"
    $failedReport="# C2 Discovery Gate Report`nschemaVersion: 1.0.0`ngeneratedAt: 2026-07-15T00:00:00Z`nsnapshotId: snapshot-pc-install-001`noperationIdentity: C2.DiscoveryCoverage.FixtureValidation`ninputFingerprint: aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa`nledgerInputFingerprint: dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd`ndiscoveryInputFingerprint: null`ngateStatus: Failed`ninputSubjectCount: 24`nnotEvaluatedInputSubjectCount: 0`ninputFailureCount: 2`noutputCandidateCount: 5`nprojectedOutputCount: 1`noutputFailureCount: 4`nissueCount: 2`nfailureAttribution: FT-05:raw-row-sha256:f1343fa48a1a370cf061818de05aa529ae846cebfb5b92360a29c2f0cd4280d6;FT-07:observation-conflict-sha256:a008931eff217b62b8d15ee99a06b095c4a68cfa01231abf0c0f2025d209a82d`nnextAllowedAction: Correct invalid observation rows and resolve observation conflicts.`n"
    ...
    $forgedFingerprint=$passed|ConvertTo-Json -Depth 100|ConvertFrom-Json -Depth 100 -DateKind String;$forgedFingerprint.discoveryArtifactFingerprint='ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff';if($loadedModule.Invoke({param($x)Test-C2SerializedPublicationResult $x},@($forgedFingerprint))[0]){throw 'forged discovery fingerprint was accepted.'}
    $missingOutputFailures=$failed|ConvertTo-Json -Depth 100|ConvertFrom-Json -Depth 100 -DateKind String;$missingOutputFailures.outputFailures=@();if($loadedModule.Invoke({param($x)Test-C2SerializedPublicationResult $x},@($missingOutputFailures))[0]){throw 'declared output failure count without rows was accepted.'}
...
if($Case -ceq 'InputAccounting'){
    ...
    function M([string]$id,[long]$bytes=0){[pscustomobject][ordered]@{id=$id;sizeBytes=$bytes}}
    function P([string]$name,[object[]]$members){[pscustomobject][ordered]@{name=$name;members=@($members)}}
    function E([string]$id,[object[]]$universe,[object[]]$partitions){[pscustomobject][ordered]@{equationId=$id;universe=@($universe);partitions=@($partitions)}}
    function S([string]$id,[object[]]$equations){[pscustomobject][ordered]@{stageId=$id;equations=@($equations)}}
    $f1=M "pc-install-primary`nSourceCorpus/PcInstall/game-data.bundle" 4096;$f2=M "pc-install-primary`nConfig/table.json" 8;$o1=M 'o1';$c1=M 'c1';$g1=M 'g1';$g2=M $conflictId
    $rawMembers=@($rawIds|ForEach-Object{M $_})
    $sp01Files=E 'Files' @($f1,$f2) @((P 'Container' @($f1)),(P 'NonContainer' @($f2)))
    $sp02Files=E 'Files' @($f1,$f2) @((P 'NotAttempted' @($f2)),(P 'Parsed' @($f1)),(P 'Opaque' @()),(P 'Failed' @()),(P 'Conflict' @()))
    $sp02Containers=E 'Containers' @($f1) @((P 'NotAttempted' @()),(P 'Parsed' @($f1)),(P 'Opaque' @()),(P 'Failed' @()),(P 'Conflict' @()))
    $sp03Observations=E 'Observations' $rawMembers @((P 'Accepted' @($rawMembers[0..3]+$rawMembers[6..7])),(P 'Rejected' @($rawMembers[4])),(P 'Excluded' @($rawMembers[5])))
    $sp03Groups=E 'CorrelationGroups' @($g1,$g2) @((P 'Resolved' @($g1)),(P 'Conflict' @($g2)))
    $sp04Objects=E 'Objects' @($o1) @((P 'Classified' @($o1)),(P 'Unclassified' @()))
    $sp05Targets=E 'Targets' @($c1,$o1) @((P 'Candidates' @($c1,$o1)),(P 'Conflicts' @()))
    $sp05Disposition=E 'Disposition' @($c1,$o1) @((P 'Parsed' @($o1)),(P 'DiscoveredOpaque' @($c1)),(P 'Encrypted' @()),(P 'RequiresRuntimeType' @()),(P 'LikelyServerDependent' @()),(P 'NotConfiguration' @()))
    $sp06Objects=E 'Objects' @($o1) @((P 'Canonicalized' @($o1)),(P 'Conflicts' @()))
    $sp06Groups=E 'Groups' @($o1) @((P 'ExactDuplicate' @()),(P 'CrossPlatformVariant' @()),(P 'Unresolved' @($o1)))
    $sp07Dispatch=E 'Dispatch' @() @((P 'Assigned' @()),(P 'RetainedForDiagnosis' @()),(P 'ConfigurationOnly' @()))
    $sp07Lanes=E 'Lanes' @() @((P 'Audio' @()),(P 'Environment' @()),(P 'Actor' @()),(P 'UI' @()),(P 'Effects' @()),(P 'Unassigned' @()))
    $stageSets=@((S 'SP-01' @($sp01Files)),(S 'SP-02' @($sp02Files,$sp02Containers)),(S 'SP-03a' @($sp03Observations)),(S 'SP-03b' @($sp03Groups)),(S 'SP-04' @($sp04Objects)),(S 'SP-05' @($sp05Targets,$sp05Disposition)),(S 'SP-06' @($sp06Objects,$sp06Groups)),(S 'SP-07' @($sp07Dispatch,$sp07Lanes)))
    function Z([string[]]$names){$h=[ordered]@{};foreach($name in $names){$h[$name]=0};[pscustomobject]$h}
    $status=[pscustomobject][ordered]@{corpus='Cataloged';extraction='NotAttempted';semantics='Unknown';unity='NotTested';disposition='RetainForLater'}
    $files=@([pscustomobject][ordered]@{snapshotId='s';sourceId='pc-install-primary';sourceKind='PcInstall';relativePath='SourceCorpus/PcInstall/game-data.bundle';containerKind='UnknownInput';sizeBytes=4096;sha256=('1'*64);capturedAt='2026-07-15T00:00:00Z';parseStatus='Parsed';disposition='RetainForLater';evidence=@();status=$status},[pscustomobject][ordered]@{snapshotId='s';sourceId='pc-install-primary';sourceKind='PcInstall';relativePath='Config/table.json';containerKind='ConfigurationCandidate';sizeBytes=8;sha256=('2'*64);capturedAt='2026-07-15T00:00:00Z';parseStatus='NotAttempted';disposition='RetainForLater';evidence=@();status=$status})
    $resolved=@([pscustomobject][ordered]@{sourceId='pc-install-primary';relativePath='SourceCorpus/PcInstall/game-data.bundle';parseStatus='Parsed';observationIds=@();evidence=@()},[pscustomobject][ordered]@{sourceId='pc-install-primary';relativePath='Config/table.json';parseStatus='NotAttempted';observationIds=@();evidence=@()})
    $projection=[pscustomobject][ordered]@{toolVersions=@();sources=@();files=$files;objects=@();configurationCandidates=@();configurationConflicts=@();canonicalGroups=@();canonicalConflicts=@();dispatchRows=@();resolvedFileResults=$resolved;mergedObjectCandidates=@();canonicalProposalProvenance=@();dispatchInputFacts=@();fileDiscoveryConflicts=@();observationConflicts=@();inputFailures=@();inputExclusions=@();inputSuppressions=@();discoveryInputs=@([pscustomobject][ordered]@{path='Tools/AssetImport/Fixtures/DiscoveryGate/valid-discovery-evidence.json';sha256=('3'*64)});coverage=[pscustomobject][ordered]@{
        files=Z @('catalogedFileCount','catalogedBytes','catalogedContainerCount','catalogedContainerBytes','nonContainerFileCount','nonContainerFileBytes','fileDiscoverySubjectCount','fileDiscoverySubjectBytes','notAttemptedFileCount','notAttemptedFileBytes','parsedFileCount','parsedFileBytes','opaqueFileCount','opaqueFileBytes','failedFileCount','failedFileBytes','fileDiscoveryConflictFileCount','fileDiscoveryConflictFileBytes');containers=Z @('notAttemptedContainerCount','notAttemptedContainerBytes','parsedContainerCount','parsedContainerBytes','opaqueContainerCount','opaqueContainerBytes','failedContainerCount','failedContainerBytes','fileDiscoveryConflictContainerCount','fileDiscoveryConflictContainerBytes');objects=Z @('objectObservationRowCount','acceptedObjectObservationRowCount','rejectedObjectObservationRowCount','excludedObjectObservationRowCount','correlationGroupCount','enumeratedObjectCount','observationConflictObjectCount','classifiedObjectCount','unclassifiedObjectCount','unresolvedDependencyCount');configuration=Z @('configurationDiscoverySubjectCount','configurationCandidateCount','configurationConflictCount','parsedConfigurationCount','discoveredOpaqueConfigurationCount','encryptedConfigurationCount','requiresRuntimeTypeConfigurationCount','likelyServerDependentConfigurationCount','notConfigurationCount');canonical=Z @('canonicalizedObjectCount','canonicalConflictObjectCount','canonicalGroupCount','exactDuplicateGroupCount','platformVariantGroupCount','unresolvedCanonicalGroupCount');dispatch=Z @('dispatchEligibleObjectCount','assignedObjectCount','retainedForDiagnosisObjectCount','configurationOnlyObjectCount','audioObjectCount','environmentObjectCount','actorObjectCount','uiObjectCount','effectsObjectCount')};decision=[pscustomobject][ordered]@{failureAttribution='None';nextAllowedAction='Continue'}}
    $projection.coverage.files.catalogedFileCount=2;$projection.coverage.files.catalogedBytes=4104;$projection.coverage.files.catalogedContainerCount=1;$projection.coverage.files.catalogedContainerBytes=4096;$projection.coverage.files.nonContainerFileCount=1;$projection.coverage.files.nonContainerFileBytes=8;$projection.coverage.files.fileDiscoverySubjectCount=2;$projection.coverage.files.fileDiscoverySubjectBytes=4104;$projection.coverage.files.notAttemptedFileCount=1;$projection.coverage.files.notAttemptedFileBytes=8;$projection.coverage.files.parsedFileCount=1;$projection.coverage.files.parsedFileBytes=4096;$projection.coverage.containers.parsedContainerCount=1;$projection.coverage.containers.parsedContainerBytes=4096
    $boundFingerprint=Get-C2DiscoveryInputFingerprint -Entries $projection.discoveryInputs
    $contractChange=[pscustomobject][ordered]@{schemaVersion='1.0.0';generatedAt='2026-07-15T00:00:00Z';requestId='C0ContractChange:C2StructuredCoverage:1.0.0';reason='Root gate summary schema 2.0.0 losslessly represents mandatory C2 structured coverage partitions.';nextAllowedAction='Provide current C2 outputs and C6-O04 to the separately authorized G5 aggregator.';inputFingerprint=('a'*64);discoveryInputFingerprint=$boundFingerprint;status='Resolved';missingProjectionFields=@();evidence=@('Tools/AssetImport/Fixtures/DiscoveryGate/valid-discovery-summary.json','docs/asset-migration/schemas/root-gate-summary.schema.json')}
    $input=[pscustomobject][ordered]@{artifactStates=@($artifactStates);rawObservationSubjects=@($rawSubjects);conflictSubjects=@($conflicts);inputFailures=@($failures);inputExclusions=@($exclusions);runtimeFacts=$runtimeFacts;contractChangeRequest=$contractChange;stageConservationSets=@($stageSets);publicProjection=$projection;discoveryInputFingerprint=$boundFingerprint}
    $result=$loadedModule.Invoke({param($x)Invoke-C2InputAccounting $x},@($input))[0]
    $a=$result.failureAccounting
    if($a.inputSubjectCount-ne24-or$a.acceptedInputSubjectCount-ne21-or$a.inputFailureCount-ne2-or$a.excludedInputSubjectCount-ne1-or$a.notEvaluatedInputSubjectCount-ne0){throw "SP-08 exact subject partition failed: $($a|ConvertTo-Json -Compress) checks=$($result.contractChecks.status-join',')"}
    ...
    $twoEquation=CloneAccounting $input;$twoEquation.stageConservationSets[0].equations[0].partitions[0].members=@();$twoEquation.stageConservationSets[1].equations[0].partitions[1].members=@($twoEquation.stageConservationSets[1].equations[0].partitions[1].members[0],$twoEquation.stageConservationSets[1].equations[0].partitions[1].members[0]);$ft10=RunAccounting $twoEquation
    ...
    $badBytes=CloneAccounting $input;$badBytes.stageConservationSets[0].equations[0].partitions[0].members[0].sizeBytes=1;$byteResult=RunAccounting $badBytes;if(($byteResult.contractChecks.status-join',')-cne'Accepted,Accepted,Accepted,Failed,NotEvaluated'){throw 'SP-01 byte conservation did not fail closed'}
    $swapped=CloneAccounting $input;$left=$swapped.stageConservationSets[0].equations[0].partitions[0].members;$swapped.stageConservationSets[0].equations[0].partitions[0].members=$swapped.stageConservationSets[0].equations[0].partitions[1].members;$swapped.stageConservationSets[0].equations[0].partitions[1].members=$left;$swapResult=RunAccounting $swapped;if(($swapResult.contractChecks.status-join',')-cne'Accepted,Accepted,Accepted,Accepted,Failed'){throw 'SP-01 semantic partition swap was accepted'}
    $badPublic=CloneAccounting $input;$badPublic.publicProjection.coverage.objects.enumeratedObjectCount=1;$publicResult=RunAccounting $badPublic;if(($publicResult.contractChecks.status-join',')-cne'Accepted,Accepted,Accepted,Accepted,Failed'){throw 'AR-S06/AR-S11 projection mismatch was accepted'}
    $badFingerprint=CloneAccounting $input;$badFingerprint.publicProjection.discoveryInputs[0].sha256=('4'*64);$fingerprintResult=RunAccounting $badFingerprint;if(($fingerprintResult.contractChecks.status-join',')-cne'Accepted,Accepted,Accepted,Accepted,Failed'){throw 'AR-S06/AR-S11 fingerprint mismatch was accepted'}
    $clean=CloneAccounting $input;$clean.rawObservationSubjects=@($clean.rawObservationSubjects|Where-Object partition -ceq 'Accepted');$clean.conflictSubjects=@();$clean.inputFailures=@();$clean.inputExclusions=@();$clean.stageConservationSets[2].equations[0].universe=@($clean.stageConservationSets[2].equations[0].partitions[0].members);$clean.stageConservationSets[2].equations[0].partitions[1].members=@();$clean.stageConservationSets[2].equations[0].partitions[2].members=@();$clean.stageConservationSets[3].equations[0].universe=@();$clean.stageConservationSets[3].equations[0].partitions[0].members=@();$clean.stageConservationSets[3].equations[0].partitions[1].members=@()
    ...
    $unparseable=CloneAccounting $clean;$i08=@($unparseable.artifactStates|Where-Object artifactId -ceq 'AR-I08')[0];$i08.readStatus='Failed';$unparseable.rawObservationSubjects=@($unparseable.rawObservationSubjects|Where-Object subjectKind -cne 'FileDiscoveryObservation');$unparseable.stageConservationSets[2].equations[0].universe=@($unparseable.stageConservationSets[2].equations[0].universe|Select-Object -First 4);$unparseable.stageConservationSets[2].equations[0].partitions[0].members=@($unparseable.stageConservationSets[2].equations[0].universe);$unparseable.inputFailures=@([pscustomobject][ordered]@{recordId='accounting-sha256:d48e97294f293a31e9feb36b9f0946b5ebd46c2911d176c704c9f30465485f31';subjectKind='ObservationDocument';subjectId='AR-I08';reasonCode='InvalidObservation';attribution='FT-05:AR-I08';evidence=@('Tools/AssetImport/Fixtures/DiscoveryGate/file-discovery-observations.json')});$documentFailure=RunAccounting $unparseable
...
$p0Entries = @(
    ...
    [pscustomobject]@{ path = 'docs/asset-migration/schemas/status-vocabulary.json'; sha256 = '9d845b2290cc606de755b2b4cc0fb877e58bc4f3964a55bec01a6ec17d8468e6' }
    [pscustomobject]@{ path = 'docs/asset-migration/schemas/root-gate-summary.schema.json'; sha256 = '8c9fdb6e50c620d1b8d8835cb6435c653b3398f5616340f5945ad643364af1ec' }
)
...
if ($Case -in @('Integration','FileFixtureIntake')) {
    ...
    $result = $loadedModule.Invoke({param($root)Invoke-C2DiscoveryIntakeGateInternal -RepositoryRoot $root -GitTransport $null -FixtureMutator $null -PublicationGeneratedAt $null},@($repositoryRoot))[0]
    ...
    if($independentD9 -cne 'e9cfb162b7aa6ade4f99548a14fe027cf43ba793fd78727f7e66b504ceb85354'){throw "Independent D9 mismatch: $independentD9"}
...
if ($Case -ceq 'ValidatorMutations') {
    ...
    $vectors=@(
        ...
        [pscustomobject]@{name='authoringPoolStatusContent';subject='AR-I05';result=(Invoke-PrivateVector AuthoringPoolStatusInvalid)},
        [pscustomobject]@{name='capabilityStatusContent';subject='AR-I05';result=(Invoke-PrivateVector CapabilityStatusInvalid)},
    ...
    "authoringPoolStatusContentMutation=Passed"
    "capabilityStatusContentMutation=Passed"
...
$expected = 'e2f0ed955ed6551462bb12fa4336b78d4ca5e2aa981c99dd707d23d5858017fb'
    ...
    foreach($member in $Ast.FindAll({param($n) $n -is [Management.Automation.Language.InvokeMemberExpressionAst]},$true)){
        ...
        if($text -match '(?i)^(\[(System\.)?IO\.|\[Diagnostics\.Process|\$process\.(Start|WaitForExit|Dispose)|\$process\.Standard(Output|Error))'){
            ...
            $allowedByFunction=@{
                ...
                'Invoke-LaneFactProjectionTest'='^\[IO\.File\]::ReadAllBytes\(\(Join-Path \$repositoryRoot ''docs/asset-migration/schemas/c2-lane-fact-package\.schema\.json''\)\)$|^\[IO\.File\]::Exists\(\$expectedPath\)$|^\[IO\.File\]::ReadAllText\(\$expectedPath,\$utf8\)$|^\[IO\.Directory\]::Exists\(\(Join-Path \$repositoryRoot ''Temp/C2DiscoveryPublication''\)\)$'
            ...
            $publicationFunctions=@('Get-C2PublicationChildPath','Write-C2DurableFile','Get-C2PublicationFileSha','Remove-C2PublicationPath','Move-C2PublicationFile','Read-C2RegisteredPublicationState','Test-C2PublishedDiscoveryGeneration','Test-C2PublishedGenerationVector','Write-C2PublicationJournal','Read-C2PublicationJournal','Invoke-C2PublicationQuarantine','Invoke-C2PublicationUnknownQuarantine','Invoke-C2PublicationRollback','Invoke-C2PublicationRecovery','Invoke-C2PublicationTransactionCore','Initialize-C2PublicationRecoveryState','Test-C2PublicationTransactionVector','Invoke-C2DiscoveryIntakeGate','Invoke-PublicationIntegrationVector')
