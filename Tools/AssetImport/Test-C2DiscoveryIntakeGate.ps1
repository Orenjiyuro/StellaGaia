[CmdletBinding()]
param(
    [ValidateSet('Pure','GitAdapter','Integration','FailureState','ValidatorMutations','FileFixtureIntake','FilePartitions','ObjectObservationPartitions','ObjectMergeProjection','ConfigurationPartitions')]
    [string]$Case = 'Pure'
)
...
if($Case -ceq 'ConfigurationPartitions'){
    ...
    function InputConfig($files,$status,$rows){[pscustomobject]@{c1Files=@($files);mergedObjects=@($merged);fileConfigurationArtifact=[pscustomobject]@{artifactPath='Tools/AssetImport/Fixtures/DiscoveryGate/file-configuration-observations.json';artifactSha256='2fd0911f90c3c210d475db07e936995de9607e11542f8d27044ad1f381f6d919';documentReadStatus=$status;document=if($status-eq'Parsed'){[pscustomobject]@{rows=@($rows)}}else{$null}}}}
    ...
    $bad=$rowB|ConvertTo-Json -Depth 10|ConvertFrom-Json -Depth 10;$bad.configurationDisposition='Encrypted';$bad.configurationObservationId=ConfigId $bad;$conflict=RunConfig (InputConfig @($f) Parsed @($rowA,$bad));if($conflict.configurationConflicts.Count-ne1-or($conflict.configurationConflicts[0].conflictingFields-join',')-cne'configurationDisposition'-or$conflict.inputFailures[0].attribution-notlike'FT-08:*'-or-not$conflict.outputsSuppressed){throw 'FT-08 disposition conflict failed'}
    $content=$rowB|ConvertTo-Json -Depth 10|ConvertFrom-Json -Depth 10;$content.contentFingerprint='7'*64;$content.configurationObservationId=ConfigId $content;$contentConflict=RunConfig (InputConfig @($f) Parsed @($rowA,$content));if($contentConflict.configurationConflicts.Count-ne1-or($contentConflict.configurationConflicts[0].conflictingFields-join',')-cne'contentFingerprint'-or$contentConflict.configurationCandidates.Count-ne1-or$contentConflict.inputFailures.Count-ne1){throw 'FT-08 content conflict failed'}
    $duplicate=RunConfig (InputConfig @($f) Parsed @($rowA,$rowA));if(@($duplicate.inputFailures|Where-Object attribution -like 'FT-05:*').Count-ne1-or$duplicate.configurationCandidates.Count-ne2-or$duplicate.inputFailures[0].subjectId-cne'raw-row-sha256:594c43a33a2f9bec09cd81ee43456661cbdefca11d23f1309c8e73946a833b2e'){throw 'duplicate HI-07 fail-closed failed'}
    $unsafe=$rowA|ConvertTo-Json -Depth 10|ConvertFrom-Json -Depth 10;$unsafe.relativePath='../table.json';$unsafe.configurationObservationId=ConfigId $unsafe;$unsafeResult=RunConfig (InputConfig @($f) Parsed @($unsafe));if(@($unsafeResult.inputFailures|Where-Object attribution -like 'FT-04:*').Count-ne1-or$unsafeResult.inputFailures[0].subjectId-cne'raw-row-sha256:368d96d73fcf2057f8fd772b4ec8a36a5662df40b7a527cedb9e0a9a5f9cdaf1'){throw 'unsafe AR-I09 row failed'}
    $unknown=$rowA|ConvertTo-Json -Depth 10|ConvertFrom-Json -Depth 10;$unknown.relativePath='Config/missing.json';$unknown.configurationObservationId=ConfigId $unknown;$unknownResult=RunConfig (InputConfig @($f) Parsed @($unknown));if(@($unknownResult.inputFailures|Where-Object reasonCode -eq 'UnknownTarget').Count-ne1){throw 'unknown AR-I09 target failed'}
    $malformed=$rowA|ConvertTo-Json -Depth 10|ConvertFrom-Json -Depth 10;$malformed.toolName='';$malformedResult=RunConfig (InputConfig @($f) Parsed @($malformed));if(@($malformedResult.inputFailures|Where-Object attribution -like 'FT-05:*').Count-ne1){throw 'malformed AR-I09 row failed'}
...
if($Case -cin @('ObjectObservationPartitions','ObjectMergeProjection')){
    ...
    if($Case -ceq 'ObjectMergeProjection'){
        ...
        $canonical=CloneInput $input;$canonical.objectObservationArtifact.document.rows=@($canonical.objectObservationArtifact.document.rows[0]);$canonical.exclusionApprovalArtifact.document.approvals=@();$canonical.objectObservationArtifact.document.rows[0].canonicalEvidence=[pscustomobject][ordered]@{memberPlatform='Pc';proposedMatchStatus='Unresolved';proposedEquivalenceFingerprint=$null;evidence=@('Tools/AssetImport/Fixtures/DiscoveryGate/Evidence/canonical.json')};$canonical.objectObservationArtifact.document.rows[0].observationId=ReId $canonical.objectObservationArtifact.document.rows[0];$canonicalResult=Run $canonical;if($canonicalResult.publicObjectCores[0].evidence -cnotcontains 'Tools/AssetImport/Fixtures/DiscoveryGate/Evidence/canonical.json'){throw 'canonical public evidence union failed'}
