...
$runtimeSource = Join-Path $projectRoot 'Assets\StellaGaia\Scripts\VASP\VisualAssetSubsetProofRunner.cs'
$builderSource = Join-Path $projectRoot 'Assets\StellaGaia\Editor\VisualAssetSubsetProofBuilder.cs'
$outputRoot = Join-Path $projectRoot 'Extracted\Validation\VASP-T1-OfflineCompile'
$runtimeAssembly = Join-Path $outputRoot 'VisualAssetSubsetProofRunner.dll'
$builderAssembly = Join-Path $outputRoot 'VisualAssetSubsetProofBuilder.dll'
...
$result = [ordered]@{
    schemaVersion = 'vasp-player-offline-compile/1.0.0'
    status = if (
...
if ($result.status -ne 'Passed') {
    throw (
        "VASP-T1 offline compile failed: runtime={0}/{1}/{2}, builder={3}/{4}/{5}" -f
...
"VASP-T1 offline compile GREEN: runtime=0/0/0 builder=0/0/0"
